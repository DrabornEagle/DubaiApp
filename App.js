...
const US_MARKET_GUIDE = [
  {
    rank: 1,
    code: "NYSE",
    name: "New York Stock Exchange",
    focus: "Blue-chip / köklü şirketler",
    type: "Hisse",
    rep: "DIA",
    note: "Kurumsal yatırımcılar ve yüksek piyasa değerli hisseler için ana merkez.",
  },
  {
    rank: 2,
    code: "NASDAQ",
    name: "NASDAQ Stock Market",
    focus: "Teknoloji / büyüme",
    type: "Hisse",
    rep: "QQQ",
    note: "Teknoloji ve büyüme hisselerinde en çok takip edilen ABD pazarı.",
  },
  {
    rank: 3,
    code: "NYSE Arca",
    ...
    type: "ETF",
    rep: "SPY",
    note: "ETF likiditesi ve fon işlemleri için en yoğun kullanılan platformlardan biri.",
  },
  {
    rank: 4,
    code: "Cboe BZX",
    name: "Cboe BZX Exchange",
    focus: "Hisse + ETF routing",
    type: "Hisse/ETF",
    rep: "VTI",
    note: "Algoritmik emir yönlendirme ve alternatif işlem akışı için sık kullanılır.",
  },
  {
    rank: 5,
    code: "NYSE American",
    name: "NYSE American (AMEX)",
    focus: "Küçük/orta ölçek",
    type: "Hisse",
    rep: "IWM",
    note: "Daha küçük piyasa değerli hisseler ve bazı ETF’lerde görünür.",
  },
  {
    rank: 6,
    code: "IEX",
    name: "Investors Exchange",
    focus: "Şeffaflık / execution kalitesi",
    type: "Hisse",
    rep: "IWM",
    note: "Daha adil emir yürütme yaklaşımı nedeniyle profesyoneller tarafından takip edilir.",
  },
  {
    rank: 7,
    code: "CME",
    name: "CME Group (CME/CBOT/NYMEX/COMEX)",
    focus: "Vadeli işlemler",
    type: "Futures",
    rep: "SPY",
    note: "S&P 500, Nasdaq, altın, petrol ve FX vadeli işlemler için kritik merkez.",
  },
  {
    rank: 8,
    code: "CBOE Options",
    name: "Cboe Options Exchange",
    focus: "Opsiyonlar / volatilite",
    type: "Options",
    rep: "QQQ",
    note: "Opsiyon stratejileri ve volatilite takibi (ör. hedge) için yoğun kullanılır.",
  },
];

const US_STOCK_PRESET_PACKS = [
  { key: "INDEX_ETF", label: "Endeks ETF", symbols: ["SPY", "QQQ", "DIA", "IWM", "VOO", "VTI", "XLK", "XLF"] },
  { key: "MEGA_CAP", label: "Mega Cap Core", symbols: ["AAPL", "MSFT", "AMZN", "GOOGL", "META", "NVDA", "BRK-B", "JPM"] },
  { key: "AI_GROWTH", label: "AI & Growth", symbols: ["NVDA", "AMD", "MSFT", "META", "AVGO", "PLTR", "AMZN", "TSLA"] },
  { key: "DIVIDEND", label: "Temettü / Defansif", symbols: ["KO", "PG", "JNJ", "PEP", "MCD", "XOM", "CVX", "VZ"] },
  { key: "BANK_ENERGY", label: "Banka + Enerji", symbols: ["JPM", "BAC", "GS", "WFC", "XOM", "CVX", "COP", "OXY"] },
];
...
function mapYahooQuoteRows(rows) {
...
async function fetchYahooQuotesDirect(list) {
  ...
  if (!res?.ok) throw new Error("stocks http " + (res?.status || "ERR"));
  const json = await res.json();
  return mapYahooQuoteRows(json?.quoteResponse?.result || []);
}

async function fetchYahooQuotesProxy(list) {
  const base = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(list.join(","))}`;
  const proxies = [
  ...
  for (const url of proxies) {
    try {
      const res = await fetchJsonWithTimeout(url, { headers: { Accept: "application/json,text/plain,*/*" } }, 15000);
      if (!res?.ok) throw new Error("proxy http " + (res?.status || "ERR"));
      const txt = await res.text();
      const json = JSON.parse(txt);
      const rows = mapYahooQuoteRows(json?.quoteResponse?.result || []);
...
const STOCK_FALLBACK_META = {
  SPY: { name: "SPDR S&P 500 ETF", exchange: "US Market", currency: "USD" },
  QQQ: { name: "Invesco QQQ Trust", exchange: "US Market", currency: "USD" },
  DIA: { name: "SPDR Dow Jones ETF", exchange: "US Market", currency: "USD" },
  IWM: { name: "iShares Russell 2000 ETF", exchange: "US Market", currency: "USD" },
  AAPL: { name: "Apple", exchange: "US Market", currency: "USD" },
  MSFT: { name: "Microsoft", exchange: "US Market", currency: "USD" },
  NVDA: { name: "NVIDIA", exchange: "US Market", currency: "USD" },
  AMZN: { name: "Amazon", exchange: "US Market", currency: "USD" },
  META: { name: "Meta Platforms", exchange: "US Market", currency: "USD" },
  GOOGL: { name: "Alphabet", exchange: "US Market", currency: "USD" },
  TSLA: { name: "Tesla", exchange: "US Market", currency: "USD" },
  AMD: { name: "AMD", exchange: "US Market", currency: "USD" },
  BRKB: { name: "Berkshire Hathaway", exchange: "US Market", currency: "USD" },
  JPM: { name: "JPMorgan Chase", exchange: "US Market", currency: "USD" },
};

function stooqSymbol(sym) {
  const s = String(sym || "").trim().toLowerCase().replace(/\./g, "-");
  return `${s}.us`;
}

async function fetchStooqQuotes(list) {
  const out = [];
  await Promise.all(
    list.map(async (sym) => {
      const url = `https://stooq.com/q/l/?s=${encodeURIComponent(stooqSymbol(sym))}&f=sd2t2ohlcvn&e=csv`;
      try {
        const res = await fetchJsonWithTimeout(url, { headers: { Accept: "text/csv,*/*" } }, 12000);
        if (!res?.ok) throw new Error("stooq http " + (res?.status || "ERR"));
        const txt = await res.text();
...
/);
        ...
        const row = {};
        for (let i = 0; i < header.length; i++) row[header[i]] = (vals[i] || "").trim();
        const symbolRaw = String(row.symbol || sym || "").toUpperCase().replace(/\.US$/, "").replace("-", ".");
        const close = Number(row.close);
        const open = Number(row.open);
        const high = Number(row.high);
        const low = Number(row.low);
        const prevClose = Number.isFinite(open) ? open : NaN;
        const price = Number.isFinite(close) ? close : NaN;
        const meta = STOCK_FALLBACK_META[String(sym || "").toUpperCase()] || {};
        if (!Number.isFinite(price)) return;
        out.push({
          symbol: symbolRaw || String(sym || "").toUpperCase(),
          name: String(row.name || meta.name || sym || "").trim(),
          price,
          change: Number.isFinite(price) && Number.isFinite(prevClose) ? price - prevClose : NaN,
          changePct: Number.isFinite(price) && Number.isFinite(prevClose) && prevClose !== 0 ? ((price - prevClose) / prevClose) * 100 : NaN,
          dayHigh: high,
          dayLow: low,
          open,
          prevClose,
          ...
          exchange: String(meta.exchange || "US Market"),
          marketState: "",
          atISO: nowISO(),
        });
      } catch {}
...
async function fetchYahooQuotes(symbols) {
  ...
  try {
    rows = await fetchYahooQuotesDirect(list);
  } catch (e) {
    lastErr = e;
  }
  ...
  if (!rows.length) {
    try {
      rows = await fetchYahooQuotesProxy(list);
    } catch (e) {
      lastErr = e;
    }
  }
  ...
  if (!rows.length) {
    try {
      rows = await fetchStooqQuotes(list);
    } catch (e) {
      lastErr = e;
    }
  }
  ...
  for (const q of rows) rowMap.set(String(q.symbol || "").toUpperCase().replace(/-/g, "."), q);

  const mapped = list.map((sym) => {
    const q = rowMap.get(sym) || rowMap.get(sym.replace(/\./g, "-"));
    if (q) return q;
    const meta = STOCK_FALLBACK_META[sym] || {};
    return {
      symbol: sym,
      name: meta.name || sym,
      price: NaN,
      ...
      exchange: meta.exchange || "US Market",
      marketState: "",
      atISO: nowISO(),
    };
  });

  if (!mapped.some((q) => Number.isFinite(q.price))) {
    throw (lastErr || new Error("Borsa verisi alınamadı"));
  }
  return mapped;
}
...
const DEFAULT_STATE = {
  ...
  expensePlanner: [],
  stockSymbols: ["SPY", "QQQ", "NVDA", "AAPL", "MSFT", "AMZN", "META", "GOOGL", "TSLA", "AMD"],
  stockQuotes: [],
  lastStocksISO: "",
  lastStocksError: "",
...
const APP_BUILD_META = {
  appName: "Dubai Journey PRO",
  versionName: "6.0.0",
  versionCode: 6,
  label: "Final v6",
  saveSchema: "v6",
...
const [tab, setTab] = useState("home");
  ...
  const [lastToastInfo, setLastToastInfo] = useState({ title: "", msg: "", at: "" });
  ...
  const homeLastNotification = useMemo(() => {
    if (lastToastInfo?.at) {
      return {
        title: lastToastInfo.title || "Bildirim",
        msg: lastToastInfo.msg || "Detay bulunamadı.",
        at: lastToastInfo.at,
      };
    }
    if (S.lastNewsNotifyISO) {
      return {
        title: "📰 Yeni Haber",
        msg: "Haber akışına yeni içerik geldi.",
        at: S.lastNewsNotifyISO,
      };
    }
    return {
      title: "Henüz bildirim yok",
      msg: "Sistem, haber ve başarı bildirimleri burada görünecek.",
      at: "",
    };
  }, [lastToastInfo, S.lastNewsNotifyISO]);
  ...
  async function refreshStocks(force) {
    ...
    try {
      ...
      if (Array.isArray(quotes) && quotes.length) {
        const validCount = quotes.filter((q) => Number.isFinite(q?.price)).length;
        setS((p) => ({ ...p, stockQuotes: quotes, stockSymbols: symbols, lastStocksISO: nowISO(), lastStocksError: validCount ? "" : (p.lastStocksError || "") }));
      } else if (force) {
        setS((p) => ({ ...p, stockSymbols: symbols, lastStocksError: p.lastStocksError || "Borsa verisi alınamadı" }));
      }
    } catch (e) {
      setS((p) => ({ ...p, lastStocksError: String(e?.message || "Borsa verisi alınamadı") }));
    }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      timer = setInterval(() => refreshStocks(false), 5 * 60 * 1000);
    })();
    return () => { if (timer) clearInterval(timer); };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  ...
  function openToast(title, msg) {
    const next = { title: title || "", msg: msg || "", at: nowISO() };
    setToastModal({ open: true, title: next.title, msg: next.msg });
    setLastToastInfo(next);
  }
  function closeToast() {
  ...
  >
    <MenuIcon theme={theme} size={20} />
                ...
                <DubaiSkylineHero
                  theme={theme}
                  S={S}
                  versionInfo={APP_BUILD_META}
                  lastNotification={homeLastNotification}
                />
              </ScrollView>
            )}
...
function DubaiSkylineHero({ theme, S, versionInfo, lastNotification }) {
  const versionText = versionInfo?.versionName || versionInfo?.label || "v6";
  const notifTitle = lastNotification?.title ? String(lastNotification.title).trim() : "Henüz yeni bildirim yok";
  const notifMsg = lastNotification?.msg ? String(lastNotification.msg).trim() : "Sistem bildirim detayları burada görünür.";
  const notifMeta = lastNotification?.at ? new Date(lastNotification.at).toLocaleString() : (S?.lastNewsNotifyISO ? new Date(S.lastNewsNotifyISO).toLocaleString() : "—");
  const lastStocks = S?.lastStocksISO ? new Date(S.lastStocksISO).toLocaleString() : "—";
  const lastWeather = S?.lastWeatherISO ? new Date(S.lastWeatherISO).toLocaleString() : "—";

  return (
    <GlassCard theme={theme} style={{ marginTop: 14, marginBottom: 4, padding: 14 }}>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>Sistem Bilgi Paneli</Text>
        <View style={{ paddingHorizontal: 10, paddingVertical: 6, borderRadius: 12, borderWidth: 1, borderColor: theme.gold, backgroundColor: "rgba(255,209,112,0.12)" }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 12 }}>Sürüm {versionText}</Text>
        </View>
      </View>

      <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
        <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 11 }}>Borsa Son Güncelleme</Text>
          <Text style={{ color: theme.text, fontWeight: "900", marginTop: 5, fontSize: 12 }} numberOfLines={1}>{lastStocks}</Text>
        </View>
        <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 11 }}>Hava Durumu Son Güncelleme</Text>
          <Text style={{ color: theme.text, fontWeight: "900", marginTop: 5, fontSize: 12 }} numberOfLines={1}>{lastWeather}</Text>
        </View>
      </View>

      <View style={{ marginTop: 12, padding: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
        <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>Gelen Son Bildirim (Haber)</Text>
        <Text style={{ color: theme.text, fontWeight: "900", marginTop: 6, lineHeight: 20 }} numberOfLines={2}>{notifTitle}</Text>
        <Text style={{ color: theme.sub, marginTop: 6, fontWeight: "800", fontSize: 12 }} numberOfLines={2}>{notifMsg}</Text>
        <Text style={{ color: theme.sub, marginTop: 6, fontWeight: "800", fontSize: 12 }} numberOfLines={1}>{notifMeta}</Text>
        {!!S?.lastStocksError && (
          <Text style={{ color: "rgba(255,99,132,0.95)", marginTop: 8, fontWeight: "900", fontSize: 12 }} numberOfLines={2}>
            Borsa Durumu: {String(S.lastStocksError)}
          </Text>
        )}
      </View>
    </GlassCard>
...
function ExpensePlannerPro({ theme, t, S, setPatch, onToast }) {
  ...
  const sorted = useMemo(() => {
    ...
    arr.sort((a, b) => {
      ...
      const pa = { "Yüksek": 0, "Orta": 1, "Düşük": 2 }[a.priority] ?? 1;
      const pb = { "Yüksek": 0, "Orta": 1, "Düşük": 2 }[b.priority] ?? 1;
  ...
  const addItem = () => {
    ...
    setPatch({ expensePlanner: [{ id: uid(), title: String(title).trim(), category, phase, currency, amount: amt, dueDate: String(dueDate || "").trim(), priority, note: String(note || "").trim(), paid: false, paidAt: "", at: nowISO() }, ...list] });
    setTitle(""); setAmount(""); setDueDate(""); setNote("");
  ...
  const addStarterTemplate = () => {
    const template = [
      ["Vize / Evrak", "Vize / Evrak", "PRE_DXB", "AED", 0, "Yüksek"],
      ["Uçak Bileti (aile)", "Uçuş", "PRE_DXB", "USD", 0, "Yüksek"],
      ["İlk 1 Ay Airbnb", "Airbnb / Konaklama", "FIRST_MONTH", "AED", 0, "Yüksek"],
      ["Ev Kirası İlk Ödeme", "Kira / Depozito", "FIRST_MONTH", "AED", 0, "Yüksek"],
      ["Okul Kayıt / İlk Taksit", "Okul", "FIRST_MONTH", "AED", 0, "Yüksek"],
      ["Ehliyet Dönüşüm", "Ehliyet / Dönüşüm", "FIRST_MONTH", "AED", 0, "Orta"],
      ["Araç Peşinatı", "Araç / Peşinat", "FIRST_MONTH", "AED", 0, "Yüksek"],
      ["Şirket Kurulum / Sanal Ofis", "Şirket Kurulum", "FIRST_MONTH", "AED", 0, "Orta"],
      ["İlk Ay Market & Yemek", "Market", "FIRST_MONTH", "AED", 0, "Orta"],
      ["Telefon / İnternet Kurulum", "Telefon/İnternet", "FIRST_MONTH", "AED", 0, "Düşük"],
    ];
    const existing = new Set(list.map((x) => `${x.title}|${x.phase}`));
    const additions = template.filter((r) => !existing.has(`${r[0]}|${r[2]}`)).map((r) => ({ id: uid(), title: r[0], category: r[1], phase: r[2], currency: r[3], amount: r[4], priority: r[5], dueDate: "", note: "", paid: false, paidAt: "", at: nowISO() }));
      ...
      <GlassCard key={item.id} theme={theme} style={{ padding: 12, marginTop: 10, opacity: item.paid ? 0.82 : 1 }}>
        <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 10 }}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={{ color: theme.sub, marginTop: 6, fontWeight: "800" }}>{item.category} • {item.priority} • {fmtMoney(item.amount || 0, 0)} {item.currency}</Text>
            <Text style={{ color: theme.sub, marginTop: 4, fontWeight: "800" }}>≈ ${fmtMoney(usd, 0)} • {Number.isFinite(tl) ? fmtMoney(tl, 0) : "—"} TL{item.dueDate ? ` • Vade: ${item.dueDate}` : ""}</Text>
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GlassCard theme={theme} style={{ padding: 14 }}>
        ...
        <Text style={{ color: theme.sub, marginTop: 6 }}>Dubai’ye gidene kadar + Dubai’de ilk ay için tüm masrafları planla, vade ekle, ödendi takibi yap.</Text>
        <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
          <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
      ...
      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        ...
        <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
          <TextInput value={dueDate} onChangeText={setDueDate} placeholder="Vade: YYYY-MM-DD" placeholderTextColor={theme.sub} style={{ flex: 1, padding: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, color: theme.text, backgroundColor: theme.glass2, fontWeight: "800" }} />
          <View style={{ flexDirection: "row", gap: 8 }}>{["Yüksek", "Orta", "Düşük"].map((p) => (<ToggleChip key={p} theme={theme} active={priority === p} text={p} onPress={() => setPriority(p)} />))}</View>
        </View>
...
function StocksProScreen({ theme, t, S, setPatch, onRefresh, onToast }) {
  ...
  const [selectedPreset, setSelectedPreset] = useState("INDEX_ETF");
  ...
  const rankedMarkets = useMemo(() => {
    return US_MARKET_GUIDE.map((m) => ({ ...m, repQuote: quoteMap.get(String(m.rep || "").toUpperCase()) || null }));
  }, [quoteMap]);

  const applyPreset = (presetKey, replaceMode = false) => {
    const preset = US_STOCK_PRESET_PACKS.find((x) => x.key === presetKey);
    if (!preset) return;
    const next = normalizeStockSymbols(replaceMode ? preset.symbols : [...(S.stockSymbols || []), ...preset.symbols]);
    setPatch({
      stockSymbols: next,
      stockQuotes: replaceMode ? (S.stockQuotes || []).filter((q) => next.includes(String(q.symbol || "").toUpperCase())) : S.stockQuotes,
    });
    setSelectedPreset(presetKey);
    onToast?.("📦", `${preset.label} listesi ${replaceMode ? "uygulandı" : "eklendi"}`);
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GlassCard theme={theme} style={{ padding: 14 }}>
        ...
        {!!S.lastStocksError && <Text style={{ color: "rgba(255,99,132,0.95)", marginTop: 6, fontWeight: "900", fontSize: 12 }}>⚠️ {S.lastStocksError}</Text>}
        <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
      ...
      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>🇺🇸 En Çok Tercih Edilen Amerikan Borsaları (Sıralı)</Text>
        <Text style={{ color: theme.sub, marginTop: 6 }}>
          Hisse/ETF/opsiyon/futures için en çok kullanılan ABD pazarları. Temsilci semboller varsa canlı veriye bağlanır.
        </Text>
        <View style={{ marginTop: 10, gap: 10 }}>
          {rankedMarkets.map((m) => {
            const q = m.repQuote;
            ...
            return (
              <View key={m.code} style={{ padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
                  <View style={{ flex: 1 }}>
                    <Text style={{ color: theme.text, fontWeight: "900" }}>{m.rank}. {m.code} • {m.name}</Text>
                    <Text style={{ color: theme.sub, marginTop: 3, fontSize: 12 }}>{m.type} • {m.focus}</Text>
                  </View>
                  {!!q && (
                    <View style={{ alignItems: "flex-end" }}>
                      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 13 }}>{q.symbol} • ${Number.isFinite(q.price) ? fmtMoney(q.price, 2) : "—"}</Text>
                      <Text style={{ color: up ? theme.green : theme.red, fontWeight: "900", fontSize: 12 }}>
                        {(q.changePct || 0) >= 0 ? "+" : ""}{Number.isFinite(q.changePct) ? fmtMoney(q.changePct, 2) : "—"}%
                      </Text>
                    </View>
                  )}
                </View>
                <Text style={{ color: theme.sub, marginTop: 6, lineHeight: 17 }}>{m.note}</Text>
              </View>
            );
          })}
        </View>
      ...
      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>🧠 Detaylı Sistem • Hazır Amerikan Watchlist Paketleri</Text>
        <Text style={{ color: theme.sub, marginTop: 6 }}>
          Sistem mantığı: (1) Pazar seç → (2) Hazır paket ekle → (3) Değişim%/Fiyat sıralaması → (4) Hacim + gün içi aralık ile teyit et.
        </Text>
        <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8, marginTop: 10 }}>
          {US_STOCK_PRESET_PACKS.map((p) => (
            <ToggleChip
              key={p.key}
              theme={theme}
        ...
        <View style={{ marginTop: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, padding: 10 }}>
          <Text style={{ color: theme.text, fontWeight: "900" }}>
            Seçili Paket: {US_STOCK_PRESET_PACKS.find((x) => x.key === selectedPreset)?.label || "—"}
          </Text>
          <Text style={{ color: theme.sub, marginTop: 6, lineHeight: 17 }}>
            {((US_STOCK_PRESET_PACKS.find((x) => x.key === selectedPreset)?.symbols) || []).join(" • ")}
          </Text>
          <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
            <TouchableOpacity onPress={() => applyPreset(selectedPreset, false)} style={{ flex: 1 }}>
              <View style={{ paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass }}>
                <Text style={{ color: theme.text, fontWeight: "900", textAlign: "center" }}>➕ Watchlist’e Ekle</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity onPress={() => applyPreset(selectedPreset, true)} style={{ flex: 1 }}>
              <View style={{ paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: theme.gold, backgroundColor: "rgba(255,209,112,0.14)" }}>
                <Text style={{ color: theme.text, fontWeight: "900", textAlign: "center" }}>♻️ Listeyi Değiştir</Text>
              </View>
            </TouchableOpacity>
          </View>
        </View>
        <Text style={{ color: theme.sub, marginTop: 10, fontSize: 12 }}>
          Not: ABD işlem saatleri yaz/kış saati uygulamasına göre değişebilir. Günlük takipte canlı fiyat + hacim + volatiliteyi birlikte yorumla.
        </Text>
      </GlassCard>
