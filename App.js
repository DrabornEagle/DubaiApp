...
// Best-effort: hide native splash ASAP so we can show our custom intro immediately.
let _SplashScreen = null;
try {
  // Works in Expo / EAS builds
...
function DubaiIntro({ onDone, durationMs = 4500, hold = false }) {
  ...
  React.useEffect(() => {
    ...
    // timer handled in separate effect
    return () => {};
  }, [W, shimmerX, pulse]);
...
useEffect(() => {
  ...
  return (
    <Pressable onPress={() => onDone?.()} style={{ flex: 1 }}>
      ...
      {/* Background */}
      <ImageBackground
        source={require("./assets/intro_bg.jpg")}
        resizeMode="cover"
...
const [tab, setTab] = useState("home");
  ...
  // Intro shows after app data is ready (prevents getting stuck on splash in standalone)
    useEffect(() => {
    // Show our JS intro instead of the OS splash.
