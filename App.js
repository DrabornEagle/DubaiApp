...
// Optional native helpers (Expo): Document Picker + Sharing (for SaveApp/LoadApp File)
let __DP_CACHE = undefined;
function getDocumentPicker() {
  if (__DP_CACHE !== undefined) return __DP_CACHE;
  try { __DP_CACHE = require("expo-document-picker"); } catch { __DP_CACHE = null; }
  return __DP_CACHE;
}
let __SHARE_CACHE = undefined;
function getSharing() {
  if (__SHARE_CACHE !== undefined) return __SHARE_CACHE;
  try { __SHARE_CACHE = require("expo-sharing"); } catch { __SHARE_CACHE = null; }
...
async function saveSnapshotToFile(name, state) {
  ...
  // 1) Android user-selected folder (SAF)
  try {
    const SAF = FS?.StorageAccessFramework;
    if (SAF?.createFileAsync && SAF?.requestDirectoryPermissionsAsync) {
      let dirUri = await kvGet(USER_SAVE_DIR_URI_KEY);
      if (!dirUri) dirUri = await getOrPickUserSaveDirUri(true);
      if (dirUri) {
        const fileUri = await SAF.createFileAsync(dirUri, filename, "application/json");
        await FS.writeAsStringAsync(fileUri, JSON.stringify(payload));
        const meta = { id: "saf:" + fileUri, name: payload.name, at: payload.at, version: payload.version };
        const idx = await loadUserSaveIndex();
        const next = [meta, ...(idx || []).filter((x) => x?.id !== meta.id)].slice(0, 60);
...
const DEFAULT_STATE = {
  ...
  gameScores: { tapBest: 0, rpsBestStreak: 0, memoryBestSec: 0 },
...
const [tab, setTab] = useState("home");
  ...
  // ✅ SaveApp (File) — no folder selection; opens system save/share UI
  async function handleSaveAppFile() {
    const nm = saveName.trim() || "SaveApp";
    const at = nowISO();
    const payload = { id: uid(), name: nm.slice(0, 40), at, version: "v6", state: stateRef.current };

    if (IS_WEB) {
      const fn = `DubaiApp_${safeFileName(payload.name)}_${at.slice(0, 19).replace(/[:T]/g, "-")}.json`;
    ...
    const FS = getFileSystem();
    if (!FS?.writeAsStringAsync) {
      openToast("⚠️", "Dosya sistemi yok. Backup Code kullan.");
      return;
    }

    const baseDir = FS.cacheDirectory || FS.documentDirectory;
    const fn = `DubaiApp_${safeFileName(payload.name)}_${at.slice(0, 19).replace(/[:T]/g, "-")}.json`;
    const path = (baseDir || FS.documentDirectory || "") + fn;

    try {
      await FS.writeAsStringAsync(path, JSON.stringify(payload));
    } catch {
      openToast("⚠️", "Dosya yazılamadı. Backup Code kullan.");
      return;
    }
    ...
    try {
      if (Sharing?.isAvailableAsync && Sharing?.shareAsync) {
        ...
        if (avail) {
          await Sharing.shareAsync(path, { mimeType: "application/json", dialogTitle: "DubaiApp Save File" });
        }
      }
    } catch {}

    openToast("✅", "Save dosyası hazır");
    setSaveLoadOpen(false);
  }

  // ✅ LoadApp (File) — opens file picker to select saved JSON and restore
  async function handleLoadAppFile() {
    ...
    const FS = getFileSystem();
    if (!DP?.getDocumentAsync || !FS?.readAsStringAsync) {
      openToast("⚠️", "Dosya seçici yok. Backup Code kullan.");
      return;
    }
    try {
  ...
  async function applyImportedText(txt) {
    ...
    if (parsed && typeof parsed === "object" && parsed.state && typeof parsed.state === "object") {
      st = parsed.state;
      importedName = String(parsed.name || "");
    } else {
      st = unpackBackupCode(parsed);
    }
    ...
    if (!merged.gameScores || typeof merged.gameScores !== "object") merged.gameScores = DEFAULT_STATE.gameScores;

    setS(merged);
    openToast("✅", importedName ? `Yüklendi: ${importedName}` : "Import başarılı (state yüklendi)");
...
function GamesHubScreen({ theme, t, S, setPatch, onToast }) {
  const [gameKey, setGameKey] = useState(null);

  const scores = S.gameScores || DEFAULT_STATE.gameScores;

  const Card = ({ icon, title, sub, onPress }) => (
    <TouchableOpacity onPress={onPress} style={{ marginTop: 12 }}>
      <GlassCard theme={theme} style={{ padding: 14 }}>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          <View style={{ flexDirection: "row", alignItems: "center", gap: 12, flex: 1 }}>
            <View style={{ width: 52, height: 52, borderRadius: 18, backgroundColor: "rgba(0,0,0,0.22)", borderWidth: 1, borderColor: theme.stroke, alignItems: "center", justifyContent: "center" }}>
              <Text style={{ fontSize: 24 }}>{icon}</Text>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>{title}</Text>
              <Text style={{ color: theme.sub, marginTop: 4, fontWeight: "800" }}>{sub}</Text>
            </View>
          </View>
          <Text style={{ color: theme.gold, fontWeight: "900", fontSize: 20 }}>›</Text>
        </View>
      </GlassCard>
    </TouchableOpacity>
  );
  ...
  if (gameKey === "tap") {
    return (
      <GameTapRush
        ...
        best={scores.tapBest || 0}
        onBest={(v) => setPatch({ gameScores: { ...scores, tapBest: Math.max(scores.tapBest || 0, v) } })}
        onToast={onToast}
      />
    );
  }
  ...
  if (gameKey === "rps") {
    return (
      <GameRPS
        theme={theme}
        onBack={() => setGameKey(null)}
        bestStreak={scores.rpsBestStreak || 0}
        onBestStreak={(v) => setPatch({ gameScores: { ...scores, rpsBestStreak: Math.max(scores.rpsBestStreak || 0, v) } })}
      />
    );
  }
  ...
  if (gameKey === "memory") {
    return (
      <GameMemoryMatch
        theme={theme}
        onBack={() => setGameKey(null)}
        bestSec={scores.memoryBestSec || 0}
        onBestSec={(v) => {
          const cur = scores.memoryBestSec || 0;
          const next = cur === 0 ? v : Math.min(cur, v);
          setPatch({ gameScores: { ...scores, memoryBestSec: next } });
        }}
        onToast={onToast}
      />
    );
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GlassCard theme={theme} style={{ padding: 14 }}>
        ...
        <Text style={{ color: theme.sub, marginTop: 6 }}>Mini oyunlar: kisa mola, kisa eglence.</Text>
        <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "900" }}>
          En iyiler • Tap Rush: <Text style={{ color: theme.gold }}>{scores.tapBest || 0}</Text> • Streak: <Text style={{ color: theme.gold }}>{scores.rpsBestStreak || 0}</Text> • Memory: <Text style={{ color: theme.gold }}>{scores.memoryBestSec ? scores.memoryBestSec + "s" : "-"}</Text>
        </Text>
      </GlassCard>

      <Card icon="⚡" title="Tap Rush" sub="30 saniye refleks • hedefe tikla" onPress={() => setGameKey("tap")} />
      <Card icon="✊" title="Tas Kagit Makas" sub="Sen vs AI • seri yap" onPress={() => setGameKey("rps")} />
      <Card icon="🧠" title="Memory Match" sub="Kartlari eslestir • sureyi dusur" onPress={() => setGameKey("memory")} />
    </ScrollView>
  );
}

function GameTopBar({ theme, title, onBack, right }) {
  return (
    <GlassCard theme={theme} style={{ padding: 12 }}>
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
        <TouchableOpacity onPress={onBack}>
          <View style={{ paddingHorizontal: 12, paddingVertical: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
            <Text style={{ color: theme.text, fontWeight: "900" }}>‹ Geri</Text>
          </View>
        </TouchableOpacity>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>{title}</Text>
...
function GameTapRush({ theme, onBack, best, onBest, onToast }) {
  ...
  const randomize = useCallback(() => {
    ...
    if (w < 80 || h < 80) return;
    const pad = 10;
    const nx = pad + Math.random() * Math.max(1, w - size - pad * 2);
    const ny = pad + Math.random() * Math.max(1, h - size - pad * 2);
    setPos({ x: nx, y: ny });
  }, [area.w, area.h]);
    ...
    const timer = setInterval(() => {
      ...
      setTimeLeft((p) => {
        if (p <= 1) {
          ...
          if (finalScore > (best || 0)) onBest?.(finalScore);
          onToast?.("🏁", "Bitti! Skor: " + finalScore);
          return 0;
        }
        return p - 1;
      });
  ...
  const s = pulse.interpolate({ inputRange: [0, 1], outputRange: [1, 1.12] });
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GameTopBar
        ...
        title="Tap Rush"
        onBack={onBack}
        right={<Text style={{ color: theme.gold, fontWeight: "900" }}>Best: {best || 0}</Text>}
      />

      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>30 saniyede ne kadar cok tiklayabilirsin?</Text>
        <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
          <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
            <Text style={{ color: theme.sub, fontWeight: "900", fontSize: 12 }}>SURE</Text>
            <Text style={{ color: theme.text, fontWeight: "900", fontSize: 22, marginTop: 4 }}>{timeLeft}s</Text>
          </View>
          <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
            <Text style={{ color: theme.sub, fontWeight: "900", fontSize: 12 }}>SKOR</Text>
            <Text style={{ color: theme.text, fontWeight: "900", fontSize: 22, marginTop: 4 }}>{score}</Text>
          </View>
          <TouchableOpacity onPress={start} style={{ justifyContent: "center" }}>
            <View style={{ paddingHorizontal: 14, paddingVertical: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.gold, backgroundColor: "rgba(255,209,112,0.18)" }}>
              <Text style={{ color: theme.text, fontWeight: "900" }}>{running ? "RUN" : "BASLA"}</Text>
            </View>
          </TouchableOpacity>
        </View>
      </GlassCard>
      ...
      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <View
          ...
          style={{ height: 340, borderRadius: 18, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(0,0,0,0.18)", overflow: "hidden" }}
        >
          <Text style={{ color: theme.sub, fontWeight: "900", position: "absolute", top: 10, left: 12 }}>
            {running ? "Hedefe tikla" : "BASLA ve hedefi yakala"}
          </Text>
          {running && (
            <TouchableOpacity onPress={hit} style={{ position: "absolute", left: pos.x, top: pos.y }}>
              <Animated.View
                style={{
                  width: size,
                  height: size,
                  ...
                  borderWidth: 2,
                  borderColor: theme.gold,
                  backgroundColor: "rgba(255,209,112,0.18)",
                  alignItems: "center",
                  justifyContent: "center",
                  transform: [{ scale: s }],
                }}
              >
                <Text style={{ color: theme.text, fontWeight: "900" }}>TAP</Text>
              </Animated.View>
            </TouchableOpacity>
          )}
        </View>
...
function GameRPS({ theme, onBack, bestStreak, onBestStreak }) {
  const choices = [
    { k: "rock", e: "✊" },
    { k: "paper", e: "🖐️" },
    { k: "scissors", e: "✌️" },
  ...
  const resultOf = (me, ai) => {
    if (me === ai) return "DRAW";
    if ((me === "rock" && ai === "scissors") || (me === "paper" && ai === "rock") || (me === "scissors" && ai === "paper")) return "WIN";
    return "LOSE";
  };

  const play = (me) => {
    const ai = choices[Math.floor(Math.random() * choices.length)].k;
    const r = resultOf(me, ai);
    setLast({ me, ai, r });
    if (r === "WIN") {
      setWins((p) => p + 1);
      setStreak((p) => {
        const next = p + 1;
        if (next > (bestStreak || 0)) onBestStreak?.(next);
        return next;
      });
    } else if (r === "LOSE") {
      setLoss((p) => p + 1);
  ...
  const label = (k) => (k === "rock" ? "Tas" : k === "paper" ? "Kagit" : "Makas");
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GameTopBar
        ...
        onBack={onBack}
        right={<Text style={{ color: theme.gold, fontWeight: "900" }}>Best: {bestStreak || 0}</Text>}
      />

      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Streak yap, rekor kir.</Text>
        <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
          <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
            <Text style={{ color: theme.sub, fontWeight: "900", fontSize: 12 }}>STREAK</Text>
            <Text style={{ color: theme.text, fontWeight: "900", fontSize: 22, marginTop: 4 }}>{streak}</Text>
          </View>
          <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
            <Text style={{ color: theme.sub, fontWeight: "900", fontSize: 12 }}>W/L/D</Text>
            <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16, marginTop: 6 }}>{wins}/{loss}/{draw}</Text>
          </View>
        </View>

        {last && (
          <View style={{ marginTop: 12, padding: 12, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.06)" }}>
            <Text style={{ color: theme.text, fontWeight: "900" }}>Sen: {label(last.me)}  •  AI: {label(last.ai)}</Text>
            <Text style={{ color: last.r === "WIN" ? theme.green : last.r === "LOSE" ? theme.red : theme.sub, fontWeight: "900", marginTop: 6 }}>
              {last.r === "WIN" ? "KAZANDIN" : last.r === "LOSE" ? "KAYBETTIN" : "BERABERE"}
            </Text>
          </View>
        )}
      </GlassCard>
      ...
      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>Secimini yap</Text>
        <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
          {choices.map((c) => (
            <TouchableOpacity key={c.k} onPress={() => play(c.k)} style={{ flex: 1 }}>
              <View style={{ paddingVertical: 14, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, alignItems: "center" }}>
                <Text style={{ fontSize: 28 }}>{c.e}</Text>
                <Text style={{ color: theme.sub, fontWeight: "900", marginTop: 6 }}>{label(c.k)}</Text>
              </View>
            </TouchableOpacity>
          ))}
        </View>
...
function GameMemoryMatch({ theme, onBack, bestSec, onBestSec, onToast }) {
  const icons = ["🦅","🐉","💎","🌙","⚡","🎯","🧠","🏙️"];
  ...
  const newGame = () => {
    ...
    const cards = shuffle([...pairs, ...pairs]).map((v, idx) => ({ id: "c" + idx, v, matched: false }));
    setDeck(cards);
    setOpenIds([]);
    setMoves(0);
    setSec(0);
  ...
  useEffect(() => { newGame(); }, []); // start once

  useEffect(() => {
    if (!running) return;
    const timer = setInterval(() => setSec((p) => p + 1), 1000);
  ...
  useEffect(() => {
    ...
    if (deck.length && deck.every((c) => c.matched)) {
      setRunning(false);
      onBestSec?.(sec);
      onToast?.("🏆", "Tamamlandi! Sure: " + sec + "s, Hamle: " + moves);
    }
  }, [deck, running, sec, moves, onBestSec, onToast]);
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GameTopBar
        ...
        title="Memory Match"
        onBack={onBack}
        right={<Text style={{ color: theme.gold, fontWeight: "900" }}>Best: {bestSec ? bestSec + "s" : "-"}</Text>}
      />

      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Ayni ikonlari eslestir.</Text>
        <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
          <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
            <Text style={{ color: theme.sub, fontWeight: "900", fontSize: 12 }}>SURE</Text>
            <Text style={{ color: theme.text, fontWeight: "900", fontSize: 20, marginTop: 4 }}>{sec}s</Text>
          </View>
          <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
            <Text style={{ color: theme.sub, fontWeight: "900", fontSize: 12 }}>HAMLE</Text>
            <Text style={{ color: theme.text, fontWeight: "900", fontSize: 20, marginTop: 4 }}>{moves}</Text>
          </View>
          <TouchableOpacity onPress={newGame} style={{ justifyContent: "center" }}>
            <View style={{ paddingHorizontal: 14, paddingVertical: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.gold, backgroundColor: "rgba(255,209,112,0.18)" }}>
              <Text style={{ color: theme.text, fontWeight: "900" }}>YENI</Text>
            </View>
          </TouchableOpacity>
        </View>
      </GlassCard>
      ...
      <View style={{ marginTop: 12, flexDirection: "row", flexWrap: "wrap", gap: 10, justifyContent: "space-between" }}>
        {deck.map((c) => {
          const open = isOpen(c.id) || c.matched;
          return (
            <TouchableOpacity key={c.id} onPress={() => onPick(c.id)} style={{ width: (DEVICE_W - 14 * 2 - 10 * 3) / 4 }}>
              <View style={{ height: 76, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: open ? "rgba(255,255,255,0.10)" : theme.glass2, alignItems: "center", justifyContent: "center" }}>
                <Text style={{ fontSize: 26, opacity: open ? 1 : 0.15 }}>{c.v}</Text>
              </View>
            </TouchableOpacity>
          );
        })}
      ...
      {!running && deck.length > 0 && deck.every((c) => c.matched) && (
        <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>Tebrikler!</Text>
          <Text style={{ color: theme.sub, marginTop: 6 }}>Sure: {sec}s • Hamle: {moves}</Text>
        </GlassCard>
      )}
    </ScrollView>
  );
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        { k: "games", label: t.games || "Oyunlar", icon: "🎮", sub: "Mini oyunlar • Eğlence modu" },
    ...
    <Modal transparent visible={open} animationType="fade" onRequestClose={onClose}>
      <Pressable onPress={onClose} style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.55)", padding: 16, justifyContent: "center" }}>
        <Pressable onPress={() => {}} style={{ maxHeight: "92%", backgroundColor: "rgba(10,16,24,0.94)", borderRadius: 18, padding: 16, borderWidth: 1, borderColor: theme.stroke }}>
          ...
          <Text style={{ color: theme.sub, marginTop: 8, lineHeight: 18 }}>
            SaveApp (File) ile dosya kaydet. LoadApp (File) ile telefondan dosya secip geri yukle.
          </Text>

          <Text style={{ color: theme.sub, marginTop: 12, fontWeight: "900" }}>Save adi</Text>
          <TextInput
            value={saveName}
            onChangeText={setSaveName}
            placeholder="Orn: Dubai_Final"
            placeholderTextColor={theme.sub}
            style={{
