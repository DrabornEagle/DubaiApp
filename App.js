...
========================= */
const STORAGE_KEY = "draborneagle_dubai_journey_PRO_v6";
...
const KV_DIR_NAME = "dubai_journey_kv_v6/";
...
========================= */
const SAVE_FILE_DIR_NAME = "dubai_journey_saves_v6/";
...
const DEFAULT_STATE = {
  ...
  todos: [],
  expenseItems: [],
  stocks: { quotes: [], lastISO: "", loading: false, error: "" },
  weather: null,
  lastWeatherISO: "",
  goldGramTl: null,
  lastGoldISO: "",
...
const [tab, setTab] = useState("home");
  ...
  const [expForm, setExpForm] = useState({ section: "Dubai Öncesi", title: "", amount: "", currency: "USD", dueDate: "", note: "", status: "PLAN" });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!Array.isArray(merged.expenseItems) || merged.expenseItems.length === 0) {
        merged.expenseItems = buildDefaultExpensePlan();
      }
      if (!merged.stocks || typeof merged.stocks !== "object") merged.stocks = { quotes: [], lastISO: "", loading: false, error: "" };
      setS(merged);
  ...
  async function refreshStocks(force = false) {
    const last = stateRef.current?.stocks?.lastISO ? new Date(stateRef.current.stocks.lastISO).getTime() : 0;
    if (!force && Date.now() - last < 1000 * 60 * 5) return;
    setS((p) => ({ ...p, stocks: { ...(p.stocks || {}), loading: true, error: "" } }));
    try {
      const syms = ["%5EGSPC","%5EIXIC","%5EDJI","AAPL","MSFT","NVDA","AMZN","TSLA","META"];
      ...
      const rows = (j?.quoteResponse?.result || []).map((q) => ({
        ...
        cur: q.currency || "USD",
        t: q.regularMarketTime ? new Date(q.regularMarketTime * 1000).toISOString() : new Date().toISOString(),
        ex: q.fullExchangeName || q.exchange || "US",
      }));
      setS((p) => ({ ...p, stocks: { quotes: rows, lastISO: new Date().toISOString(), loading: false, error: "" } }));
    } catch (e) {
      setS((p) => ({ ...p, stocks: { ...(p.stocks || {}), loading: false, error: "Borsa verisi alınamadı" } }));
    }
  }
  ...
  useEffect(() => {
    if (boot) return;
    refreshStocks(false);
    const id = setInterval(() => refreshStocks(false), 1000 * 60 * 10);
    return () => clearInterval(id);
  }, [boot]);

  function openExpenseForm(item = null) {
    if (item) {
      setExpEditId(item.id);
      setExpForm({
        section: item.section || "Dubai Öncesi", title: item.title || "", amount: String(item.amount ?? ""), currency: item.currency || "USD", dueDate: item.dueDate || "", note: item.note || "", status: item.status || "PLAN"
      });
    } else {
      setExpEditId(null);
      setExpForm({ section: "Dubai Öncesi", title: "", amount: "", currency: "USD", dueDate: "", note: "", status: "PLAN" });
    }
  ...
  function saveExpenseItem() {
    ...
    const amt = Number(String(expForm.amount || "").replace(",", "."));
    if (!title) return openToast("⚠️", "Masraf adı gerekli");
    if (!Number.isFinite(amt) || amt < 0) return openToast("⚠️", "Tutar geçerli değil");
    const item = {
      id: expEditId || uid(),
      section: expForm.section || "Dubai Öncesi",
      title, amount: amt, currency: expForm.currency || "USD", dueDate: String(expForm.dueDate || "").trim(),
      note: String(expForm.note || "").trim(), status: expForm.status || "PLAN", updatedAt: new Date().toISOString()
    };
    setS((p) => {
      const list = Array.isArray(p.expenseItems) ? [...p.expenseItems] : [];
      const i = list.findIndex((x) => x.id === item.id);
      if (i >= 0) list[i] = item; else list.unshift(item);
      return { ...p, expenseItems: list };
    });
    setExpModalOpen(false);
    openToast("✅", expEditId ? "Masraf güncellendi" : "Masraf eklendi");
  }

  function deleteExpenseItem(id) {
    setS((p) => ({ ...p, expenseItems: (p.expenseItems || []).filter((x) => x.id !== id) }));
...
return (
          ...
          <View style={{ flex: 1, paddingHorizontal: 14, paddingTop: 10, paddingBottom: 92, opacity: boot ? 0.5 : 1 }}>
            {tab === "home" && (
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 14 }}>
                <DubaiPremiumBackdropMini theme={theme} />
  ...
  >
    <MenuIcon theme={theme} size={20} />
            ...
            {tab === "stocks" && (
              <StocksScreen theme={theme} t={t} S={S} onRefresh={() => refreshStocks(true)} />
            )}
            ...
            {tab === "expenses" && (
              <ExpensesPlannerScreen
                theme={theme}
                t={t}
                items={S.expenseItems || []}
                usdToTl={S.usdToTl || 0}
                onAdd={() => openExpenseForm()}
                onEdit={openExpenseForm}
                onDelete={deleteExpenseItem}
                onBulkSeed={() => setS((p)=> ({...p, expenseItems: buildDefaultExpensePlan()}))}
              />
            )}
      ...
      <Modal visible={expModalOpen} transparent animationType="slide" onRequestClose={() => setExpModalOpen(false)}>
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "flex-end" }}>
          <View style={{ maxHeight: DEVICE_H * 0.82, borderTopLeftRadius: 22, borderTopRightRadius: 22, backgroundColor: "#0B101A", borderWidth: 1, borderColor: theme.stroke, padding: 14 }}>
            <ScrollView showsVerticalScrollIndicator={false}>
              <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>{expEditId ? "Masraf Düzenle" : "Masraf Ekle"}</Text>
              {[
                ["Başlık", "title", "Örn: Airbnb ilk ay"],
                ["Tutar", "amount", "Örn: 2500"],
                ["Tarih", "dueDate", "YYYY-MM-DD (opsiyonel)"],
                ["Not", "note", "Açıklama (opsiyonel)"],
              ].map(([label,key,ph]) => (
                <View key={key} style={{ marginTop: 10 }}>
                  <Text style={{ color: theme.sub, marginBottom: 6 }}>{label}</Text>
                  <TextInput value={String(expForm[key] ?? "")} onChangeText={(v)=> setExpForm(f=> ({...f,[key]: v}))} placeholder={ph} placeholderTextColor={theme.sub} keyboardType={key==='amount'?'numeric':'default'} style={{ color: theme.text, borderWidth:1, borderColor: theme.stroke, borderRadius: 14, padding: 12, backgroundColor:"rgba(255,255,255,0.03)" }} />
                </View>
              ))}
              <View style={{ marginTop: 10 }}>
                <Text style={{ color: theme.sub, marginBottom: 6 }}>Bölüm</Text>
                <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                  {["Dubai Öncesi","Dubai İlk Ay","Sonrası / Opsiyonel"].map(v => <ToggleChip key={v} theme={theme} active={expForm.section===v} text={v} onPress={()=> setExpForm(f=> ({...f, section:v}))} />)}
                </View>
              </View>
              <View style={{ marginTop: 10 }}>
                <Text style={{ color: theme.sub, marginBottom: 6 }}>Para Birimi</Text>
                <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
                  {["USD","AED","TL"].map(v => <ToggleChip key={v} theme={theme} active={expForm.currency===v} text={v} onPress={()=> setExpForm(f=> ({...f, currency:v}))} />)}
                </View>
              </View>
              <View style={{ marginTop: 10 }}>
                <Text style={{ color: theme.sub, marginBottom: 6 }}>Durum</Text>
                <View style={{ flexDirection: "row", gap: 8, flexWrap: "wrap" }}>
                  {["PLAN","DUE","PAID"].map(v => <ToggleChip key={v} theme={theme} active={expForm.status===v} text={v} onPress={()=> setExpForm(f=> ({...f, status:v}))} />)}
                </View>
              </View>
              <View style={{ flexDirection: "row", gap: 10, marginTop: 14, marginBottom: 10 }}>
                <GlowButtonDubaiShimmer theme={theme} kind="green" text="Kaydet" onPress={saveExpenseItem} shimmer={stripeAnim} />
                <GlowButtonDubaiShimmer theme={theme} kind="orange" text="Kapat" onPress={() => setExpModalOpen(false)} shimmer={stripeAnim} />
              </View>
            </ScrollView>
          </View>
        </View>
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#070A12" }}>
      ...
      <ImageBackground source={require("./assets/splash.png")} resizeMode="cover" style={{ flex: 1 }}>
        <View style={{ flex: 1, backgroundColor: "rgba(7,10,18,0.18)" }} />
      </ImageBackground>
    </View>
  );
}

function buildDefaultExpensePlan() {
  const rows = [
    ["Dubai Öncesi", "Pasaport / Evrak yenileme", 150, "USD", "PLAN"],
    ["Dubai Öncesi", "Uçak biletleri (aile)", 1200, "USD", "PLAN"],
    ["Dubai Öncesi", "Türkiye çıkış / hazırlık masrafları", 500, "USD", "PLAN"],
    ["Dubai Öncesi", "Dubai ehliyet dönüşüm evrakları", 350, "USD", "PLAN"],
    ["Dubai Öncesi", "Şirket / oturum ön hazırlık", 1500, "USD", "PLAN"],
    ["Dubai İlk Ay", "Airbnb / geçici konaklama", 2500, "USD", "PLAN"],
    ["Dubai İlk Ay", "Yeme-içme", 900, "USD", "PLAN"],
    ["Dubai İlk Ay", "Ulaşım / taksi / metro", 450, "USD", "PLAN"],
    ["Dubai İlk Ay", "Okul başvuru / kayıt ön ödemeleri", 1200, "USD", "PLAN"],
    ["Dubai İlk Ay", "Telefon / internet / hat", 200, "USD", "PLAN"],
    ["Dubai İlk Ay", "Ev depozito + ilk ödemeler", 6000, "USD", "PLAN"],
    ["Dubai İlk Ay", "Beklenmeyen gider tamponu", 2000, "USD", "PLAN"],
  ];
  return rows.map((r, i) => ({ id: `expseed_${i+1}`, section: r[0], title: r[1], amount: r[2], currency: r[3], status: r[4], dueDate: "", note: "", updatedAt: new Date().toISOString() }));
}

function DubaiPremiumBackdropMini({ theme }) {
  const beam = useRef(new Animated.Value(-120)).current;
  useEffect(() => {
    const a = Animated.loop(Animated.sequence([
      Animated.timing(beam, { toValue: DEVICE_W + 120, duration: 7000, easing: Easing.linear, useNativeDriver: true }),
      Animated.timing(beam, { toValue: -120, duration: 0, useNativeDriver: true }),
      Animated.delay(1200),
    ])); a.start(); return () => a.stop();
  }, []);
  return (
    <View style={{ height: 110, marginBottom: 10, borderRadius: 22, overflow: "hidden", borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.02)" }}>
      <View style={{ position: "absolute", inset: 0, backgroundColor: "rgba(3,6,16,0.65)" }} />
      {[0.06,0.14,0.22,0.34,0.48,0.61,0.74,0.86].map((x,i)=>(<View key={i} style={{ position:"absolute", bottom: 0, left: `${x*100}%`, width: 16 + (i%3)*8, height: 28 + (i%4)*14, backgroundColor: "rgba(255,255,255,0.06)", borderTopLeftRadius: 8, borderTopRightRadius: 8, borderWidth:1, borderColor:"rgba(255,209,112,0.08)" }} />))}
      <View style={{ position:"absolute", bottom:0, left:"49%", width:8, height:82, backgroundColor:"rgba(255,209,112,0.20)", borderTopLeftRadius: 8, borderTopRightRadius: 8 }} />
      <Animated.View pointerEvents="none" style={{ position:"absolute", top:-10, width: 70, height: 140, backgroundColor:"rgba(78,168,255,0.15)", transform:[{ translateX: beam }, { rotate: "12deg" }] }} />
      <View style={{ position:"absolute", left:14, top:12 }}>
        <Text style={{ color: theme.text, fontWeight: "900" }}>Dubai Skyline HUD</Text>
        <Text style={{ color: theme.sub, marginTop: 4, fontWeight: "800", fontSize: 12 }}>Animasyonlu premium arka plan modülü</Text>
      </View>
    </View>
  );
}

function StocksScreen({ theme, S, onRefresh }) {
  const quotes = S?.stocks?.quotes || [];
  const loading = !!S?.stocks?.loading;
  const err = S?.stocks?.error || "";
  const groups = { Endeksler: quotes.filter(q => String(q.symbol).startsWith("^")), Hisseler: quotes.filter(q => !String(q.symbol).startsWith("^")) };
  const Card = ({ q }) => {
    const up = (q.chg || 0) >= 0;
    return (
      <View style={{ padding: 12, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.05)", marginTop: 10 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <View style={{ flex: 1, paddingRight: 8 }}>
            <Text style={{ color: theme.text, fontWeight: "900" }}>{q.symbol}</Text>
            <Text numberOfLines={1} style={{ color: theme.sub, marginTop: 3, fontWeight: "800", fontSize: 12 }}>{q.name}</Text>
          </View>
          <View style={{ alignItems: "flex-end" }}>
            <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>{fmtMoney(q.price, 2)} {q.cur}</Text>
            <Text style={{ color: up ? theme.green : theme.red, fontWeight: "900", marginTop: 3 }}>
              {up ? "+" : ""}{fmtMoney(q.chg, 2)} • {up ? "+" : ""}{fmtMoney(q.chgPct, 2)}%
            </Text>
          </View>
        </View>
      </View>
      ...
      <GlassCard theme={theme} style={{ padding: 14 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <View>
            ...
            <Text style={{ color: theme.sub, marginTop: 6, fontWeight: "800" }}>S&P 500 • Nasdaq • Dow Jones + popüler hisseler</Text>
          </View>
          <TouchableOpacity onPress={onRefresh}><Text style={{ color: theme.gold, fontWeight: "900" }}>Yenile</Text></TouchableOpacity>
        </View>
        {!!S?.stocks?.lastISO && <Text style={{ color: theme.sub, marginTop: 10, fontSize: 12 }}>Son güncelleme: {new Date(S.stocks.lastISO).toLocaleString()}</Text>}
        {loading && <Text style={{ color: theme.sub, marginTop: 8 }}>Veriler yükleniyor…</Text>}
        {!!err && <Text style={{ color: theme.red, marginTop: 8 }}>{err}</Text>}
      </GlassCard>
      {Object.entries(groups).map(([title, arr]) => (
        <GlassCard key={title} theme={theme} style={{ padding: 12, marginTop: 12 }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>{title}</Text>
          {(arr.length ? arr : []).map((q) => <Card key={q.symbol} q={q} />)}
          {!arr.length && <Text style={{ color: theme.sub, marginTop: 10 }}>Veri yok</Text>}
        </GlassCard>
      ))}
    </ScrollView>
  );
...
function ExpensesPlannerScreen({ theme, items, usdToTl, onAdd, onEdit, onDelete, onBulkSeed }) {
  const sections = ["Dubai Öncesi", "Dubai İlk Ay", "Sonrası / Opsiyonel"];
  const list = Array.isArray(items) ? items : [];
  const sum = (arr, status=null) => arr.filter(x => !status || x.status===status).reduce((a,x)=> a + Number(x.amount||0), 0);
  const usdToTry = Number(usdToTl||0) || 0;
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GlassCard theme={theme} style={{ padding: 14 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <View style={{ flex: 1, paddingRight: 8 }}>
            ...
            <Text style={{ color: theme.sub, marginTop: 6, fontWeight: "800" }}>Dubai’ye gidene kadar + ilk ay detaylı plan takip sistemi</Text>
          </View>
          <TouchableOpacity onPress={onAdd}><Text style={{ color: theme.gold, fontWeight: "900" }}>+ Ekle</Text></TouchableOpacity>
        </View>
        <View style={{ flexDirection: "row", gap: 10, marginTop: 12, flexWrap: "wrap" }}>
          <ToggleChip theme={theme} active={false} text="Hazır Planı Yükle" onPress={onBulkSeed} />
          <ToggleChip theme={theme} active={false} text={`Toplam: ${fmtMoney(sum(list),0)} USD`} onPress={()=>{}} />
          {!!usdToTry && <ToggleChip theme={theme} active={false} text={`≈ ${fmtMoney(sum(list)*usdToTry,0)} TL`} onPress={()=>{}} />}
        </View>
      </GlassCard>
      {sections.map(sec => {
        const arr = list.filter(x => (x.section||"")===sec);
        return (
          <GlassCard key={sec} theme={theme} style={{ padding: 12, marginTop: 12 }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>{sec}</Text>
              <Text style={{ color: theme.gold, fontWeight: "900" }}>{fmtMoney(sum(arr),0)} USD</Text>
            </View>
            <Text style={{ color: theme.sub, marginTop: 6, fontSize: 12 }}>Ödendi: {fmtMoney(sum(arr, "PAID"),0)} • Planlanan: {fmtMoney(sum(arr, "PLAN"),0)} • Bekliyor: {fmtMoney(sum(arr, "DUE"),0)}</Text>
            {!arr.length && <Text style={{ color: theme.sub, marginTop: 10 }}>Henüz kayıt yok</Text>}
            {arr.map(it => {
              const color = it.status === "PAID" ? theme.green : it.status === "DUE" ? theme.orange : theme.gold;
              return (
                <TouchableOpacity key={it.id} onPress={() => onEdit(it)} onLongPress={() => onDelete(it.id)} style={{ marginTop: 10 }}>
                  <View style={{ padding: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.04)" }}>
                    <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
                      <Text style={{ color: theme.text, fontWeight: "900", flex: 1, paddingRight: 6 }}>{it.title}</Text>
                      <Text style={{ color, fontWeight: "900" }}>{fmtMoney(it.amount,0)} {it.currency || "USD"}</Text>
                    </View>
                    <Text style={{ color: theme.sub, marginTop: 6, fontSize: 12 }}>Durum: {it.status} {it.dueDate ? `• Tarih: ${it.dueDate}` : ""}</Text>
                    {!!it.note && <Text style={{ color: theme.sub, marginTop: 4, fontSize: 12 }}>{it.note}</Text>}
                  </View>
                </TouchableOpacity>
              );
            })}
      ...
      <Text style={{ color: theme.sub, marginTop: 10, textAlign: "center", fontSize: 12 }}>Dokun: düzenle • Uzun bas: sil</Text>
    </ScrollView>
  );
}
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        { k: "stocks", label: t.stocks || "Borsalar", icon: "📈", sub: "ABD endeksleri • Hisseler" },
      ],
    },
    {
      title: "Araçlar",
      items: [
        { k: "expenses", label: t.expenseList || "Masraflar Listesi", icon: "💸", sub: "Dubai öncesi + ilk ay planı" },
