// App.js — Dubai Journey PRO v6 (Single-file • No external dependencies)
...
const DEFAULT_STATE = {
  ...
  notes: [],
  todos: [],
  costItems: defaultCostItemsTemplate(),
  weather: null,
  lastWeatherISO: "",
  goldGramTl: null,
  lastGoldISO: "",
  ...
  return (
    <Pressable onPress={() => onDone?.()} style={{ flex: 1, backgroundColor: "#05070B" }}>
      ...
      <Image
        ...
        resizeMode="cover"
      />
      <View style={{ position: "absolute", left: 0, right: 0, top: 0, bottom: 0, backgroundColor: "rgba(5,7,11,0.25)" }} />
      {/* Background */}
      ...
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center", paddingHorizontal: 18 }}>
        ...
        >
          <Text style={{ color: "rgba(255,255,255,0.85)", fontSize: 12, letterSpacing: 3, textAlign: "center" }}>
...
const [tab, setTab] = useState("home");
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!Array.isArray(merged.costItems)) merged.costItems = DEFAULT_STATE.costItems;
...
return (
  ...
  >
    <MenuIcon theme={theme} size={20} />
                ...
                <HomeDubaiHero theme={theme} progress={progress} daysLeft={daysLeftToTravel} />
...
/* =========================
   Move Costs Planner + US Stocks (NEW v6)
========================= */
function defaultCostItemsTemplate() {
  const now = new Date();
  const y = now.getFullYear();
  const mk = (m, d) => `${y}-${pad2(m)}-${pad2(d)}`;
  const arr = [
    // Dubai öncesi
    { phase: "before", category: "Evrak", title: "Pasaport / yenileme / noter çeviri", currency: "TL", amount: 0, qty: 1, dueDate: mk(4, 15), priority: "Yüksek", paid: false, note: "" },
    { phase: "before", category: "Vize", title: "Dubai giriş / oturum başlangıç evrakları", currency: "AED", amount: 0, qty: 1, dueDate: mk(8, 15), priority: "Yüksek", paid: false, note: "" },
    { phase: "before", category: "Sağlık", title: "Seyahat sağlık sigortası", currency: "AED", amount: 0, qty: 1, dueDate: mk(9, 1), priority: "Orta", paid: false, note: "" },
    { phase: "before", category: "Uçuş", title: "Aile uçak biletleri (3 kişi)", currency: "USD", amount: 0, qty: 1, dueDate: mk(10, 15), priority: "Yüksek", paid: false, note: "" },
    { phase: "before", category: "Konaklama", title: "İlk ay Airbnb / geçici konaklama rezervasyon", currency: "AED", amount: 0, qty: 1, dueDate: mk(11, 10), priority: "Yüksek", paid: false, note: "" },
    { phase: "before", category: "Okul", title: "Çocuk okul araştırma / kayıt başvuru ücretleri", currency: "AED", amount: 0, qty: 1, dueDate: mk(10, 20), priority: "Orta", paid: false, note: "" },
    { phase: "before", category: "Şirket", title: "Sanal ofisli şirket kurulum başlangıç bütçesi", currency: "AED", amount: 0, qty: 1, dueDate: mk(10, 25), priority: "Orta", paid: false, note: "" },
    { phase: "before", category: "Araç", title: "Dubai ehliyet dönüşüm / evrak hazırlığı", currency: "AED", amount: 0, qty: 1, dueDate: mk(11, 5), priority: "Orta", paid: false, note: "" },

    // İlk ay Dubai
    { phase: "firstMonth", category: "Konaklama", title: "Airbnb 1. ay kira/konaklama", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 1), priority: "Yüksek", paid: false, note: "" },
    { phase: "firstMonth", category: "Yeme-İçme", title: "Market + yemek (ilk ay)", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 2), priority: "Yüksek", paid: false, note: "" },
    { phase: "firstMonth", category: "Ulaşım", title: "Taksi/metro/otobüs (ilk ay)", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 2), priority: "Orta", paid: false, note: "" },
    { phase: "firstMonth", category: "Telefon", title: "SIM + internet paketi", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 2), priority: "Orta", paid: false, note: "" },
    { phase: "firstMonth", category: "Resmi İşlem", title: "Emirates ID / medical / biometrics", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 5), priority: "Yüksek", paid: false, note: "" },
    { phase: "firstMonth", category: "Ehliyet", title: "Türkiye ehliyetini Dubai ehliyetine çevirme", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 7), priority: "Yüksek", paid: false, note: "" },
    { phase: "firstMonth", category: "Uber", title: "Uber/RTA sınav ve izin işlemleri", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 10), priority: "Yüksek", paid: false, note: "" },
    { phase: "firstMonth", category: "Ev", title: "Kalıcı ev için depozito + komisyon + ilk peşin", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 15), priority: "Yüksek", paid: false, note: "" },
    { phase: "firstMonth", category: "Okul", title: "Okul kayıt / depozito / ilk taksit", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 18), priority: "Yüksek", paid: false, note: "" },
    { phase: "firstMonth", category: "Araç", title: "Araç kiralama / peşinat / teslim masrafı", currency: "AED", amount: 0, qty: 1, dueDate: mk(12, 20), priority: "Yüksek", paid: false, note: "" },
    { phase: "firstMonth", category: "Acil Durum", title: "Beklenmeyen gider tamponu", currency: "USD", amount: 0, qty: 1, dueDate: mk(12, 25), priority: "Orta", paid: false, note: "" },
  ];
  return arr.map((x, i) => ({
    id: "cost_" + (i + 1) + "_" + Math.random().toString(36).slice(2, 7),
    createdAt: nowISO(),
    ...x,
  }));
}
function costItemUnitAmount(it) {
  const a = Number(it?.amount || 0);
  return Number.isFinite(a) ? a : 0;
}
function costItemQty(it) {
  const q = Number(it?.qty || 1);
  return Number.isFinite(q) && q > 0 ? q : 1;
}
function costItemTotal(it) {
  return costItemUnitAmount(it) * costItemQty(it);
}
function costToUsd(it, usdToAed, usdToTl) {
  const total = costItemTotal(it);
  const cur = String(it?.currency || "USD").toUpperCase();
  if (cur === "USD") return total;
  if (cur === "AED") return total / (Number(usdToAed) || 3.6725);
  if (cur === "TL" || cur === "TRY") return (Number(usdToTl) || 0) ? total / Number(usdToTl) : 0;
  return total;
}
function phaseLabelTR(k) {
  return k === "before" ? "Dubai Öncesi" : "Dubai İlk Ay";
}
function phaseLabelEN(k) {
  return k === "before" ? "Before Dubai" : "First Month in Dubai";
}
function phaseLabel(k, lang) {
  return lang === "EN" ? phaseLabelEN(k) : phaseLabelTR(k);
}
function priorityEmoji(p) {
  if (p === "Yüksek" || p === "High") return "🔴";
  if (p === "Düşük" || p === "Low") return "🟢";
  return "🟡";
}

function SplashHoldScreen() {
  return (
    <View style={{ flex: 1, backgroundColor: "#070A12" }}>
      <StatusBar hidden />
      <ImageBackground source={require("./assets/splash.png")} resizeMode="cover" style={{ flex: 1 }}>
        <View style={{ flex: 1, backgroundColor: "rgba(7,10,18,0.12)", alignItems: "center", justifyContent: "center" }}>
          <View style={{ width: 150, height: 5, borderRadius: 99, backgroundColor: "rgba(255,255,255,0.10)", overflow: "hidden" }}>
            <Animated.View
              style={{
                width: "65%",
                height: 5,
                borderRadius: 99,
                backgroundColor: "rgba(244,208,122,0.72)",
              }}
            />
          </View>
        </View>
...
function HomeDubaiHero({ theme, progress = 0, daysLeft = 0 }) {
  const sweep = useRef(new Animated.Value(-260)).current;
  const pulse = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    const a = Animated.loop(
      Animated.sequence([
        Animated.timing(sweep, { toValue: 260, duration: 2400, easing: Easing.inOut(Easing.quad), useNativeDriver: true }),
        Animated.timing(sweep, { toValue: -260, duration: 0, useNativeDriver: true }),
        Animated.delay(900),
      ])
    );
    const b = Animated.loop(
  ...
  const bars = [
    [14, 34, 36], [42, 26, 52], [70, 18, 30], [92, 24, 76], [124, 22, 44],
    [153, 18, 35], [177, 30, 68], [214, 22, 49], [242, 16, 33], [264, 30, 58]
  ];

  return (
    <GlassCard theme={theme} style={{ marginBottom: 12, padding: 12, overflow: "hidden" }}>
      <Animated.View
        style={{
          position: "absolute",
          width: 190,
      ...
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        <View style={{ flex: 1, paddingRight: 10 }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>Dubai Skyline Motion HUD</Text>
          <Text style={{ color: theme.sub, marginTop: 4, fontWeight: "800" }}>
            İkonik Dubai tema • canlı neon hareket • ana sayfa premium görünüm
          </Text>
        </View>
        <View style={{ alignItems: "flex-end" }}>
          <Text style={{ color: theme.gold, fontWeight: "900", fontSize: 18 }}>{Math.round(clamp(progress, 0, 1) * 100)}%</Text>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>{daysLeft} gün</Text>
        </View>
      </View>

      <View style={{ marginTop: 12, borderRadius: 18, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(0,0,0,0.18)", overflow: "hidden", height: 124 }}>
        <View style={{ position: "absolute", left: 0, right: 0, top: 0, height: 38, backgroundColor: "rgba(255,255,255,0.02)" }} />
        <View style={{ position: "absolute", left: 10, right: 10, top: 44, height: 1, backgroundColor: "rgba(255,255,255,0.07)" }} />
        <View style={{ position: "absolute", left: 10, right: 10, top: 78, height: 1, backgroundColor: "rgba(255,255,255,0.05)" }} />
        ...
        <Animated.View
          style={{
            ...
            width: 70,
            backgroundColor: "rgba(255,255,255,0.06)",
            transform: [{ translateX: sweep }, { skewX: "-14deg" }],
          }}
        />
        ...
        {bars.map((b, i) => (
          <View
            ...
            style={{
              ...
              borderTopRightRadius: 8,
              borderWidth: 1,
              borderColor: "rgba(244,208,122,0.14)",
              backgroundColor: i % 2 ? "rgba(255,255,255,0.05)" : "rgba(76,190,255,0.05)",
            }}
          />
        ))}

        <View style={{ position: "absolute", left: 141, bottom: 12, width: 8, height: 88, borderRadius: 4, backgroundColor: "rgba(244,208,122,0.28)" }} />
        <View style={{ position: "absolute", left: 144, bottom: 100, width: 2, height: 10, backgroundColor: "rgba(244,208,122,0.7)" }} />
      </View>
    </GlassCard>
  );
}

async function fetchUsStocksBoard() {
  const symbols = [
    "^GSPC","^DJI","^IXIC","SPY","QQQ","NVDA","AAPL","MSFT","AMZN","GOOGL","META","TSLA","AMD","NFLX","PLTR"
  ];
  const url = "https://query1.finance.yahoo.com/v7/finance/quote?symbols=" + encodeURIComponent(symbols.join(","));
  const res = await fetch(url, { headers: { "cache-control": "no-cache" } });
  if (!res.ok) throw new Error("stocks http " + res.status);
  const json = await res.json();
  const rows = (json?.quoteResponse?.result || []).map((r) => ({
    symbol: r.symbol,
    name: r.shortName || r.longName || r.displayName || r.symbol,
    marketState: r.marketState || "",
    price: Number(r.regularMarketPrice ?? r.postMarketPrice ?? 0),
    chg: Number(r.regularMarketChange ?? 0),
    chgPct: Number(r.regularMarketChangePercent ?? 0),
  ...
  return {
    updatedAt: nowISO(),
    indices: ["^GSPC","^DJI","^IXIC"].map((s) => by[s]).filter(Boolean),
    etfs: ["SPY","QQQ"].map((s) => by[s]).filter(Boolean),
    watch: ["NVDA","AAPL","MSFT","AMZN","GOOGL","META","TSLA","AMD","NFLX","PLTR"].map((s) => by[s]).filter(Boolean),
    raw: rows,
  };
}
...
function quoteTone(theme, q) {
  return (q?.chgPct || 0) >= 0 ? theme.green : theme.red;
}
function StockQuoteCard({ theme, q, big }) {
  if (!q) return null;
  const tone = quoteTone(theme, q);
  const rngPct = q.high && q.low && q.high > q.low && q.price
    ...
    >
      <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
        <View style={{ flex: 1, paddingRight: 8 }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: big ? 15 : 14 }}>{q.symbol}</Text>
          <Text numberOfLines={1} style={{ color: theme.sub, fontWeight: "700", marginTop: 2, fontSize: 11 }}>
            {q.name}
          </Text>
        </View>
        <View style={{ alignItems: "flex-end" }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: big ? 18 : 16 }}>
            {Number.isFinite(q.price) ? q.price.toLocaleString("en-US", { maximumFractionDigits: 2 }) : "—"}
          </Text>
          <Text style={{ color: tone, fontWeight: "900", marginTop: 2 }}>
            {(q.chg || 0) >= 0 ? "+" : ""}{(Number(q.chg || 0)).toFixed(2)} ({(q.chgPct || 0) >= 0 ? "+" : ""}{(Number(q.chgPct || 0)).toFixed(2)}%)
          </Text>
        </View>
      </View>
      <View style={{ marginTop: 8, height: 7, borderRadius: 99, backgroundColor: "rgba(255,255,255,0.08)", overflow: "hidden" }}>
        <View style={{ width: `${rngPct}%`, height: 7, borderRadius: 99, backgroundColor: "rgba(255,209,112,0.45)" }} />
      </View>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 8 }}>
        <Text style={{ color: theme.sub, fontWeight: "700", fontSize: 11 }}>Low: {q.low || "—"}</Text>
        <Text style={{ color: theme.sub, fontWeight: "700", fontSize: 11 }}>Vol: {q.volume ? formatCompact(q.volume) : "—"}</Text>
        <Text style={{ color: theme.sub, fontWeight: "700", fontSize: 11 }}>High: {q.high || "—"}</Text>
      </View>
    </View>
  );
}
function USStocksScreen({ theme, t }) {
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");
  const [board, setBoard] = useState({ updatedAt: "", indices: [], etfs: [], watch: [] });

  const refresh = useCallback(async () => {
    setErr("");
    setLoading((p) => (board.updatedAt ? false : true) || p);
    try {
      const data = await fetchUsStocksBoard();
      setBoard(data);
      setErr("");
    } catch (e) {
      setErr(String(e?.message || "Borsa verisi alınamadı"));
    } finally {
      setLoading(false);
    }
  }, [board.updatedAt]);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => { if (alive) refresh(); }, 60 * 1000);
    return () => { alive = false; clearInterval(timer); };
  }, [refresh]);

  const topGainers = useMemo(() => [...(board.watch || [])].sort((a, b) => (b.chgPct || 0) - (a.chgPct || 0)).slice(0, 3), [board.watch]);
  const topLosers = useMemo(() => [...(board.watch || [])].sort((a, b) => (a.chgPct || 0) - (b.chgPct || 0)).slice(0, 3), [board.watch]);
  const breadth = useMemo(() => {
    const list = board.watch || [];
    const up = list.filter((x) => (x.chgPct || 0) > 0).length;
    const down = list.filter((x) => (x.chgPct || 0) < 0).length;
    const flat = Math.max(0, list.length - up - down);
    return { up, down, flat };
  }, [board.watch]);
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GlassCard theme={theme} style={{ padding: 14 }}>
        <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
          <View style={{ flex: 1, paddingRight: 8 }}>
            ...
            <Text style={{ color: theme.sub, marginTop: 5, fontWeight: "800" }}>
              Amerikan endeksleri + takip listesi (canlı yenileme ~60 sn)
            </Text>
          </View>
          <TouchableOpacity onPress={refresh}>
            <Chip theme={theme} text={t.refresh} />
          </TouchableOpacity>
        </View>

        <View style={{ marginTop: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(0,0,0,0.18)", padding: 12 }}>
          <Text style={{ color: theme.text, fontWeight: "900" }}>Piyasa Breadth</Text>
          <Text style={{ color: theme.sub, marginTop: 6 }}>
            Yükselen: <Text style={{ color: theme.green, fontWeight: "900" }}>{breadth.up}</Text> •
            Düşen: <Text style={{ color: theme.red, fontWeight: "900" }}> {breadth.down}</Text> •
            Yatay: <Text style={{ color: theme.gold, fontWeight: "900" }}> {breadth.flat}</Text>
          </Text>
          <Text style={{ color: theme.sub, marginTop: 6, fontSize: 12 }}>
            {board.updatedAt ? `Güncellendi: ${new Date(board.updatedAt).toLocaleTimeString()}` : "Henüz veri yok"}
          </Text>
          {!!err && <Text style={{ color: theme.red, marginTop: 6, fontWeight: "800" }}>{err}</Text>}
        </View>
      </GlassCard>

      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>Ana Endeksler</Text>
        <View style={{ marginTop: 10 }}>
          {(board.indices || []).map((q) => <StockQuoteCard key={q.symbol} theme={theme} q={q} big />)}
          {loading && !(board.indices || []).length && <Text style={{ color: theme.sub }}>Yükleniyor…</Text>}
        </View>
      </GlassCard>

      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>ETF Referans</Text>
        <View style={{ marginTop: 10 }}>
          {(board.etfs || []).map((q) => <StockQuoteCard key={q.symbol} theme={theme} q={q} />)}
        </View>
      </GlassCard>

      <View style={{ flexDirection: "row", gap: 12, marginTop: 12 }}>
        <GlassCard theme={theme} style={{ flex: 1, padding: 12 }}>
          <Text style={{ color: theme.text, fontWeight: "900" }}>Top Gainers</Text>
          <View style={{ marginTop: 8 }}>
            {topGainers.map((q) => (
              <Text key={q.symbol} style={{ color: theme.sub, marginBottom: 6, fontWeight: "800" }}>
                <Text style={{ color: theme.text }}>{q.symbol}</Text> •{" "}
                <Text style={{ color: theme.green, fontWeight: "900" }}>{(q.chgPct || 0).toFixed(2)}%</Text>
              </Text>
            ))}
          </View>
        </GlassCard>
        <GlassCard theme={theme} style={{ flex: 1, padding: 12 }}>
          <Text style={{ color: theme.text, fontWeight: "900" }}>Top Losers</Text>
          <View style={{ marginTop: 8 }}>
            {topLosers.map((q) => (
              <Text key={q.symbol} style={{ color: theme.sub, marginBottom: 6, fontWeight: "800" }}>
                <Text style={{ color: theme.text }}>{q.symbol}</Text> •{" "}
                <Text style={{ color: theme.red, fontWeight: "900" }}>{(q.chgPct || 0).toFixed(2)}%</Text>
              </Text>
            ))}
          </View>
        </GlassCard>
      ...
      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>Takip Listesi</Text>
        <Text style={{ color: theme.sub, marginTop: 4 }}>NVDA, AAPL, MSFT, AMZN, GOOGL, META, TSLA, AMD, NFLX, PLTR</Text>
        <View style={{ marginTop: 10 }}>
          {(board.watch || []).map((q) => <StockQuoteCard key={q.symbol} theme={theme} q={q} />)}
        </View>
      </GlassCard>
    </ScrollView>
  );
...
function MoveCostsPlannerScreen({ theme, t, S, setPatch }) {
  const list = Array.isArray(S.costItems) ? S.costItems : [];
  ...
  const [fPhase, setFPhase] = useState("before");
  const [fCat, setFCat] = useState("Diğer");
  const [fAmount, setFAmount] = useState("");
  const [fQty, setFQty] = useState("1");
  const [fCurrency, setFCurrency] = useState("AED");
  const [fDue, setFDue] = useState("");
  const [fPriority, setFPriority] = useState("Orta");
  const [fNote, setFNote] = useState("");

  const usdToAed = Number(S.usdToAed) || 3.6725;
  const usdToTl = Number(S.usdToTl) || 0;
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((it) => {
      ...
      if (statusFilter === "pending" && it.paid) return false;
      if (!qq) return true;
      return [it.title, it.category, it.note, it.currency].join(" ").toLowerCase().includes(qq);
    }).sort((a, b) => {
  ...
  }, [list, q, phaseFilter, statusFilter]);

  const summary = useMemo(() => {
    const phases = ["before", "firstMonth"];
    const out = {};
    let totalUsd = 0;
    let paidUsd = 0;
    for (const ph of phases) {
      const items = list.filter((x) => x.phase === ph);
      const phaseUsd = items.reduce((s, x) => s + costToUsd(x, usdToAed, usdToTl), 0);
      const phasePaidUsd = items.filter((x) => x.paid).reduce((s, x) => s + costToUsd(x, usdToAed, usdToTl), 0);
      out[ph] = { items, usd: phaseUsd, paidUsd: phasePaidUsd };
      totalUsd += phaseUsd;
      paidUsd += phasePaidUsd;
    }
    return { ...out, totalUsd, paidUsd, pendingUsd: Math.max(0, totalUsd - paidUsd) };
  }, [list, usdToAed, usdToTl]);

  const catTotals = useMemo(() => {
    const m = new Map();
    for (const it of list) {
      const k = it.category || "Diğer";
      m.set(k, (m.get(k) || 0) + costToUsd(it, usdToAed, usdToTl));
    }
    return Array.from(m.entries()).map(([k, v]) => ({ k, v })).sort((a, b) => b.v - a.v).slice(0, 6);
  }, [list, usdToAed, usdToTl]);
  ...
  function openEdit(it) {
    setEditId(it.id);
    setFTitle(it.title || "");
    setFPhase(it.phase || "before");
    setFCat(it.category || "Diğer");
    setFAmount(String(Number(it.amount || 0) || ""));
    setFQty(String(Number(it.qty || 1) || 1));
    setFCurrency(it.currency || "AED");
    setFDue(it.dueDate || "");
    setFPriority(it.priority || "Orta");
    setFNote(it.note || "");
    setModalOpen(true);
  }
  function saveItem() {
    const title = String(fTitle || "").trim();
    if (!title) return Alert.alert("Eksik", "Başlık gir.");
    const prev = editId ? list.find((z) => z.id === editId) : null;
    const item = {
      id: editId || uid(),
      createdAt: prev?.createdAt || nowISO(),
      title,
      phase: fPhase,
      category: String(fCat || "Diğer").trim() || "Diğer",
      amount: toNum(fAmount),
      qty: Math.max(1, toInt(fQty) || 1),
      currency: fCurrency,
      dueDate: String(fDue || "").trim(),
      priority: fPriority,
      paid: prev?.paid || false,
      note: String(fNote || "").trim(),
    };
    setPatch({
      costItems: editId
        ? list.map((x) => (x.id === editId ? { ...x, ...item } : x))
        : [{ ...item }, ...list],
    });
    setModalOpen(false);
  }
  function togglePaid(id) {
    setPatch({ costItems: list.map((x) => (x.id === id ? { ...x, paid: !x.paid } : x)) });
  }
  function deleteItem(id) {
    Alert.alert("Kalem silinsin mi?", "Bu masraf kalemi silinecek.", [
      { text: "Vazgeç", style: "cancel" },
      { text: "Sil", style: "destructive", onPress: () => setPatch({ costItems: list.filter((x) => x.id !== id) }) },
    ]);
  }
  function restoreTemplateIfEmpty() {
    if (list.length > 0) {
      Alert.alert("Template ekle", "Mevcut liste korunacak ve örnek kalemler eklenecek.", [
        { text: "Vazgeç", style: "cancel" },
        { text: "Ekle", onPress: () => setPatch({ costItems: [...list, ...defaultCostItemsTemplate()] }) },
      ]);
    } else {
      setPatch({ costItems: defaultCostItemsTemplate() });
    }
  }
    ...
    <View style={{ flex: 1 }}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
        <GlassCard theme={theme} style={{ padding: 14 }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
            <View style={{ flex: 1, paddingRight: 10 }}>
              <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>🧾 {t.costsList || "Masraflar Listesi"}</Text>
              <Text style={{ color: theme.sub, marginTop: 5, fontWeight: "800" }}>
                Dubai öncesi + Dubai ilk ay için detaylı masraf planlama ve takip sistemi
              </Text>
            </View>
            <TouchableOpacity onPress={openNew}>
              <Chip theme={theme} text="+ Kalem" />
            </TouchableOpacity>
          </View>

          <View style={{ marginTop: 12, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(0,0,0,0.18)", padding: 12 }}>
            <Text style={{ color: theme.text, fontWeight: "900" }}>USD Özet (yaklaşık)</Text>
            <Text style={{ color: theme.sub, marginTop: 6 }}>
              Toplam: <Text style={{ color: theme.gold, fontWeight: "900" }}>${fmtMoney(summary.totalUsd, 0)}</Text> •
              Ödenen: <Text style={{ color: theme.green, fontWeight: "900" }}> ${fmtMoney(summary.paidUsd, 0)}</Text> •
              Kalan: <Text style={{ color: theme.red, fontWeight: "900" }}> ${fmtMoney(summary.pendingUsd, 0)}</Text>
            </Text>
            <View style={{ marginTop: 10, height: 8, borderRadius: 99, backgroundColor: "rgba(255,255,255,0.08)", overflow: "hidden" }}>
              <View style={{ width: `${summary.totalUsd ? clamp((summary.paidUsd / summary.totalUsd) * 100, 0, 100) : 0}%`, height: 8, borderRadius: 99, backgroundColor: "rgba(123,255,178,0.55)" }} />
            </View>

            <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
              {["before","firstMonth"].map((ph) => (
                <View key={ph} style={{ flex: 1, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.03)", padding: 10 }}>
                  <Text style={{ color: theme.text, fontWeight: "900", fontSize: 12 }}>{phaseLabel(ph, S.lang)}</Text>
                  <Text style={{ color: theme.gold, fontWeight: "900", marginTop: 6 }}>${fmtMoney(summary?.[ph]?.usd || 0, 0)}</Text>
                  <Text style={{ color: theme.sub, marginTop: 2, fontSize: 11 }}>
                    {summary?.[ph]?.items?.length || 0} kalem • Ödenen ${fmtMoney(summary?.[ph]?.paidUsd || 0, 0)}
                  </Text>
                </View>
              ))}
            </View>
          ...
          <TextInput
            value={q}
            onChangeText={setQ}
            placeholder="Ara: ev, araç, okul, vize..."
            placeholderTextColor={theme.sub}
            style={[inp(theme), { marginTop: 12 }]}
          />

          <View style={{ flexDirection: "row", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
            <ToggleChip theme={theme} active={phaseFilter === "all"} text="Tüm Fazlar" onPress={() => setPhaseFilter("all")} />
            <ToggleChip theme={theme} active={phaseFilter === "before"} text="Dubai Öncesi" onPress={() => setPhaseFilter("before")} />
            <ToggleChip theme={theme} active={phaseFilter === "firstMonth"} text="İlk Ay" onPress={() => setPhaseFilter("firstMonth")} />
          </View>
          <View style={{ flexDirection: "row", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
            <ToggleChip theme={theme} active={statusFilter === "all"} text="Tümü" onPress={() => setStatusFilter("all")} />
            <ToggleChip theme={theme} active={statusFilter === "pending"} text="Bekleyen" onPress={() => setStatusFilter("pending")} />
            <ToggleChip theme={theme} active={statusFilter === "paid"} text="Ödenen" onPress={() => setStatusFilter("paid")} />
            <TouchableOpacity onPress={restoreTemplateIfEmpty} style={{ paddingHorizontal: 10, paddingVertical: 8 }}>
              <Text style={{ color: theme.gold, fontWeight: "900" }}>Template Ekle</Text>
            </TouchableOpacity>
          </View>
        </GlassCard>
        ...
        {!!catTotals.length && (
          <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
            <Text style={{ color: theme.text, fontWeight: "900" }}>Kategori Bazlı Toplam (USD)</Text>
            <View style={{ marginTop: 10 }}>
              {catTotals.map((x) => (
                <View key={x.k} style={{ marginBottom: 8 }}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                    <Text style={{ color: theme.sub, fontWeight: "800" }}>{x.k}</Text>
                    <Text style={{ color: theme.gold, fontWeight: "900" }}>${fmtMoney(x.v, 0)}</Text>
                  </View>
                  <View style={{ marginTop: 5, height: 6, borderRadius: 99, backgroundColor: "rgba(255,255,255,0.06)", overflow: "hidden" }}>
                    <View style={{ width: `${summary.totalUsd ? clamp((x.v / summary.totalUsd) * 100, 0, 100) : 0}%`, height: 6, backgroundColor: "rgba(244,208,122,0.45)" }} />
                  </View>
                </View>
              ))}
            </View>
        ...
        <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>Masraf Kalemleri</Text>
          <Text style={{ color: theme.sub, marginTop: 4 }}>{filtered.length} / {list.length} kalem gösteriliyor</Text>

          <View style={{ marginTop: 10 }}>
            {!filtered.length && <Text style={{ color: theme.sub }}>Liste boş. “+ Kalem” ile ekleyebilirsin.</Text>}

            {filtered.map((it) => {
              const total = costItemTotal(it);
              const usdEq = costToUsd(it, usdToAed, usdToTl);
              return (
                <View key={it.id} style={{ borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.04)", padding: 12, marginBottom: 10 }}>
                  <View style={{ flexDirection: "row", alignItems: "center" }}>
                    <TouchableOpacity onPress={() => togglePaid(it.id)} style={{ marginRight: 10 }}>
                      <View style={{ width: 24, height: 24, borderRadius: 8, borderWidth: 1, borderColor: it.paid ? theme.green : theme.stroke, backgroundColor: it.paid ? "rgba(123,255,178,0.20)" : "rgba(255,255,255,0.03)", alignItems: "center", justifyContent: "center" }}>
                        <Text style={{ color: it.paid ? theme.green : theme.sub, fontWeight: "900" }}>{it.paid ? "✓" : ""}</Text>
                      </View>
                    </TouchableOpacity>
                    <View style={{ flex: 1, paddingRight: 8 }}>
                      <Text style={{ color: theme.text, fontWeight: "900" }}>{it.title}</Text>
                      <Text style={{ color: theme.sub, marginTop: 4, fontWeight: "800", fontSize: 12 }}>
                        {phaseLabel(it.phase, S.lang)} • {it.category || "Diğer"} • {priorityEmoji(it.priority)} {it.priority || "Orta"}
                      </Text>
                      {!!it.dueDate && <Text style={{ color: theme.sub, marginTop: 2, fontSize: 12 }}>Tarih: {it.dueDate}</Text>}
                    </View>
                    <View style={{ alignItems: "flex-end" }}>
                      <Text style={{ color: theme.text, fontWeight: "900" }}>{fmtMoney(total, 0)} {it.currency || "AED"}</Text>
                      <Text style={{ color: theme.gold, fontWeight: "900", marginTop: 4 }}>${fmtMoney(usdEq, 0)}</Text>
                    </View>
                  </View>

                  {!!it.note && <Text style={{ color: theme.sub, marginTop: 8 }}>{it.note}</Text>}

                  <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 10 }}>
                    <View style={{ flexDirection: "row", gap: 8 }}>
                      <TouchableOpacity onPress={() => openEdit(it)} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.04)" }}>
                        <Text style={{ color: theme.text, fontWeight: "900" }}>Düzenle</Text>
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => deleteItem(it.id)} style={{ paddingHorizontal: 12, paddingVertical: 8, borderRadius: 12, borderWidth: 1, borderColor: "rgba(255,93,115,0.35)", backgroundColor: "rgba(255,93,115,0.08)" }}>
                        <Text style={{ color: theme.red, fontWeight: "900" }}>Sil</Text>
                      </TouchableOpacity>
                    </View>
                    <Text style={{ color: it.paid ? theme.green : theme.gold, fontWeight: "900", alignSelf: "center" }}>
      ...
      <Modal transparent visible={modalOpen} animationType="fade" onRequestClose={() => setModalOpen(false)}>
        <View style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.55)", justifyContent: "center", padding: 16 }}>
          <GlassCard theme={theme} style={{ padding: 14, maxHeight: "88%" }}>
            <ScrollView showsVerticalScrollIndicator={false}>
              <Text style={{ color: theme.text, fontWeight: "900", fontSize: 17 }}>{editId ? "Masraf Kalemi Düzenle" : "Yeni Masraf Kalemi"}</Text>

              <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800" }}>Başlık</Text>
              <TextInput value={fTitle} onChangeText={setFTitle} placeholder="Örn: Emirates ID + Medical" placeholderTextColor={theme.sub} style={[inp(theme), { marginTop: 6 }]} />

              <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800" }}>Faz</Text>
              <View style={{ flexDirection: "row", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
                <ToggleChip theme={theme} active={fPhase === "before"} text="Dubai Öncesi" onPress={() => setFPhase("before")} />
                <ToggleChip theme={theme} active={fPhase === "firstMonth"} text="İlk Ay" onPress={() => setFPhase("firstMonth")} />
              </View>

              <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800" }}>Kategori</Text>
              <TextInput value={fCat} onChangeText={setFCat} placeholder="Araç / Okul / Vize / Ev..." placeholderTextColor={theme.sub} style={[inp(theme), { marginTop: 6 }]} />

              <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: theme.sub, fontWeight: "800" }}>Tutar (birim)</Text>
                  <TextInput value={fAmount} onChangeText={(v) => setFAmount(moneyOnly(v))} keyboardType="decimal-pad" placeholder="0" placeholderTextColor={theme.sub} style={[inp(theme), { marginTop: 6 }]} />
                </View>
                <View style={{ width: 90 }}>
                  <Text style={{ color: theme.sub, fontWeight: "800" }}>Adet</Text>
                  <TextInput value={fQty} onChangeText={(v) => setFQty(digitsOnly(v))} keyboardType="number-pad" placeholder="1" placeholderTextColor={theme.sub} style={[inp(theme), { marginTop: 6 }]} />
                </View>
              </View>

              <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800" }}>Para Birimi</Text>
              <View style={{ flexDirection: "row", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
                {["USD","AED","TL"].map((c) => (
                  <ToggleChip key={c} theme={theme} active={fCurrency === c} text={c} onPress={() => setFCurrency(c)} />
                ))}
              </View>

              <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800" }}>Planlanan Tarih (YYYY-MM-DD)</Text>
              <TextInput value={fDue} onChangeText={setFDue} placeholder="2026-12-10" placeholderTextColor={theme.sub} style={[inp(theme), { marginTop: 6 }]} />

              <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800" }}>Öncelik</Text>
              <View style={{ flexDirection: "row", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
                {["Düşük","Orta","Yüksek"].map((p) => (
                  <ToggleChip key={p} theme={theme} active={fPriority === p} text={p} onPress={() => setFPriority(p)} />
                ))}
              </View>

              <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800" }}>Not</Text>
              <TextInput value={fNote} onChangeText={setFNote} placeholder="İsteğe bağlı not..." placeholderTextColor={theme.sub} multiline style={[inp(theme), { marginTop: 6, minHeight: 80, textAlignVertical: "top" }]} />

              <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
                <TouchableOpacity onPress={() => setModalOpen(false)} style={{ flex: 1, paddingVertical: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, alignItems: "center", backgroundColor: "rgba(255,255,255,0.04)" }}>
                  <Text style={{ color: theme.sub, fontWeight: "900" }}>{t.cancel}</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={saveItem} style={{ flex: 1, paddingVertical: 12, borderRadius: 14, borderWidth: 1, borderColor: "rgba(255,209,112,0.35)", alignItems: "center", backgroundColor: "rgba(255,209,112,0.10)" }}>
                  <Text style={{ color: theme.gold, fontWeight: "900" }}>{t.save}</Text>
                </TouchableOpacity>
              </View>
            </ScrollView>
          </GlassCard>
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        { k: "stocks", label: t.stocks || "Borsalar", icon: "📈", sub: "ABD endeksleri • Hisseler • Takip" },
      ],
    },
    {
      title: "Araçlar",
      items: [
        ...
        { k: "costs", label: t.costsList || "Masraflar Listesi", icon: "🧾", sub: "Dubai öncesi + ilk ay gider planı" },
      ],
    },
    {
      title: "Sistem",
