...
const STOCK_META = {
  ...
  DIA: { name: "SPDR Dow Jones ETF", exchange: "NYSE Arca" },
  ...
  NFLX: { name: "Netflix", exchange: "NASDAQ" },
  PLTR: { name: "Palantir", exchange: "NASDAQ" },
};
...
function toStooqSymbol(sym) {
  ...
  if (!raw) return "";
  if (raw.startsWith('^')) return ""; // stooq index mapping not handled here
  return raw.replace(/\./g, '-').replace(/\^/g, '') + '.us';
}
...
async function fetchStooqQuotesBatch(symbols) {
  const list = normalizeStockSymbols(symbols);
  if (!list.length) return [];
  const stooqList = list.map(toStooqSymbol).filter(Boolean);
  if (!stooqList.length) return [];
  const url = `https://stooq.com/q/l/?s=${encodeURIComponent(stooqList.join(','))}&i=d`;
  const res = await fetch(url);
  if (!res.ok) throw new Error('stooq http ' + res.status);
  const txt = await res.text();
  const lines = String(txt || '').trim().split(/\r?\n/).filter(Boolean);
  if (lines.length < 2) return [];
  const header = parseCsvLineBasic(lines[0]).map((h) => String(h || '').trim().toLowerCase());
  const idx = (k) => header.indexOf(k);
  const rows = [];
  for (let i = 1; i < lines.length; i++) {
    const cols = parseCsvLineBasic(lines[i]);
    const get = (k) => {
      const j = idx(k);
      return j >= 0 ? cols[j] : '';
    };
    const symbolRaw = String(get('symbol') || '').trim().toUpperCase();
    if (!symbolRaw || symbolRaw === 'N/D') continue;
    const baseSym = symbolRaw.replace(/\.US$/i, '').replace(/-/g, '.').toUpperCase();
    const close = safeNum(get('close'));
    const open = safeNum(get('open'));
    const high = safeNum(get('high'));
    const low = safeNum(get('low'));
    const volume = safeNum(get('volume'));
    const change = Number.isFinite(close) && Number.isFinite(open) ? close - open : NaN;
    const changePct = Number.isFinite(change) && Number.isFinite(open) && open !== 0 ? (change / open) * 100 : NaN;
    const meta = STOCK_META[baseSym] || {};
    rows.push({
      symbol: baseSym,
      name: meta.name || baseSym,
      price: close,
      change,
      changePct,
      dayHigh: high,
      dayLow: low,
      open,
      prevClose: Number.isFinite(open) ? open : NaN,
      volume,
      marketCap: NaN,
      currency: 'USD',
  ...
  rows.sort((a, b) => list.indexOf(a.symbol) - list.indexOf(b.symbol));
  return rows;
}
...
async function fetchYahooQuotes(symbols) {
  ...
  // 1) Yahoo quote endpoint (best detail: mcap, change, volume, exchange)
  try {
    const url = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(list.join(","))}&lang=en-US&region=US`;
    ...
    const bySym = new Map(mapped.map((r) => [String(r.symbol || "").toUpperCase(), r]));
    const ordered = list.map((sym) => bySym.get(sym)).filter(Boolean);
    if (ordered.length) return ordered;
  } catch {}

  // 2) Fallback: Stooq (reliable for price/high/low/volume on many US symbols)
  try {
    const stooqRows = await fetchStooqQuotesBatch(list);
    if (stooqRows.length) return stooqRows;
  } catch {}

  // 3) Return cached-like empty quote shells so UI doesn't hard-fail
  return list.map((sym) => ({
    symbol: sym,
    ...
    marketCap: NaN,
    currency: 'USD',
    exchange: STOCK_META[sym]?.exchange || 'US Market',
    marketState: '',
    atISO: nowISO(),
    source: 'fallback',
  }));
...
const [tab, setTab] = useState("home");
  ...
  async function refreshStocks(force) {
    ...
    try {
      ...
      if (force) {
        const symbols = normalizeStockSymbols(stateRef.current?.stockSymbols || DEFAULT_STATE.stockSymbols);
        setS((p) => ({ ...p, stockQuotes: (p.stockQuotes || []).length ? p.stockQuotes : [], stockSymbols: symbols, lastStocksISO: nowISO() }));
      }
    } finally {
...
function DubaiSkylineHero({ theme, S }) {
  const latestNews = Array.isArray(S?.news) && S.news.length ? S.news[0] : null;
  const lastNotifyISO = S?.lastNewsNotifyISO || "";
  const lastStocksISO = S?.lastStocksISO || "";
  const lastWeatherISO = S?.lastWeatherISO || "";

  const line = (label, value, accent = theme.text) => (
    <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 10, marginTop: 8 }}>
      <Text style={{ color: theme.sub, fontWeight: "800", flex: 1 }}>{label}</Text>
      <Text style={{ color: accent, fontWeight: "900", flex: 1, textAlign: "right" }} numberOfLines={1}>{value || "—"}</Text>
    </View>
  );

  return (
    <GlassCard theme={theme} style={{ marginTop: 12, marginBottom: 6, padding: 14 }}>
      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>Sistem Bilgi Paneli</Text>
      <Text style={{ color: theme.sub, marginTop: 6, fontWeight: "800" }}>Final sürüm durumu • versiyon bilgisi • son bildirim özeti</Text>

      <View style={{ marginTop: 10, padding: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
        <Text style={{ color: theme.text, fontWeight: "900" }}>Uygulama Bilgisi</Text>
        {line("Sürüm", "v6")}
        {line("Version Code", "6")}
        {line("Kayıt Şeması", "SaveApp / Backup Code (tek dosya)")}
      </View>

      <View style={{ marginTop: 10, padding: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
        <Text style={{ color: theme.text, fontWeight: "900" }}>Son Bildirim Detayı</Text>
        {line("Bildirim Zamanı", lastNotifyISO ? new Date(lastNotifyISO).toLocaleString() : "Henüz bildirim yok")}
        {line("Son Haber Başlığı", latestNews?.title || "Henüz haber verisi yok")}
        {line("Kategori", latestNews?.kind || "—", theme.gold)}
      </View>

      <View style={{ marginTop: 10, flexDirection: "row", gap: 10 }}>
        <View style={{ flex: 1, padding: 10, borderRadius: 12, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>Borsa Senkron</Text>
          <Text style={{ color: theme.text, fontWeight: "900", marginTop: 4 }} numberOfLines={1}>{lastStocksISO ? new Date(lastStocksISO).toLocaleTimeString() : "Bekliyor"}</Text>
        </View>
        <View style={{ flex: 1, padding: 10, borderRadius: 12, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>Hava Durumu</Text>
          <Text style={{ color: theme.text, fontWeight: "900", marginTop: 4 }} numberOfLines={1}>{lastWeatherISO ? new Date(lastWeatherISO).toLocaleTimeString() : "Bekliyor"}</Text>
        </View>
      </View>
    </GlassCard>
  );
