  ...
  >
    <MenuIcon theme={theme} size={20} />
      ...
      <SaveLoadModal
        ...
        onPickFolder={async () => {
          const uri = await getOrPickUserSaveDirUri(true);
          if (uri) openToast("📁", "Kayıt klasörü seçildi");
          else openToast("⚠️", "Klasör seçilmedi");
          await refreshSaveList();
        }}
      />
          ...
          {!isWeb && (
            <View style={{ marginTop: 10 }}>
              <Text style={{ color: "rgba(255,255,255,0.55)", fontWeight: "800", fontSize: 12 }}>
                ✅ APK: “SaveApp (File)” ilk seferde kayıt klasörü seçtirir. “LoadApp” listeden seçilerek geri yüklenir.
              </Text>
              <TouchableOpacity onPress={onPickFolder} style={{ marginTop: 10 }}>
                <View style={{ paddingVertical: 12, borderRadius: 14, backgroundColor: "rgba(76,190,255,0.14)", borderWidth: 1, borderColor: theme.stroke, alignItems: "center" }}>
                  <Text style={{ color: theme.text, fontWeight: "900" }}>📁 Kayıt Klasörü Seç / Değiştir</Text>
                </View>
              </TouchableOpacity>
            </View>
          )}
