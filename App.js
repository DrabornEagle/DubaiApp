...
let __PICKER_CACHE = undefined; // undefined = not checked yet
function getDocumentPicker() {
  if (__PICKER_CACHE !== undefined) return __PICKER_CACHE;
  try {
    __PICKER_CACHE = require("expo-document-picker");
  } catch {
    __PICKER_CACHE = null;
  }
  return __PICKER_CACHE;
}
...
async function saveSnapshotToFile(name, state) {
  ...
  // Write into app sandbox first (guaranteed permission), then open “Kaydet/Paylaş” sheet
  ...
  // Open system share/save sheet (does NOT require folder permission)
  try {
    const available = await (Sharing?.isAvailableAsync ? Sharing.isAvailableAsync() : Promise.resolve(false));
    if (available && Sharing?.shareAsync) {
      await Sharing.shareAsync(path, {
        mimeType: "application/json",
        dialogTitle: "DubaiApp Kaydını Kaydet",
        UTI: "public.json",
      });
    }
  } catch {
    // Even if share fails, file is still created in sandbox
  }

  return { id: "share:" + filename, name: payload.name, at: payload.at, version: payload.version };
}
...
const DEFAULT_STATE = {
  ...
  gameScores: { tapRush: 0, reactionBestMs: null, quickMath: 0, rpsWins: 0, memoryBestSec: null, luckySpin: 0 },
...
const [tab, setTab] = useState("home");
  ...
  // ✅ Final: Save/Load (File Picker + Share Sheet)
  async function handleSaveAppFile() {
    const nm = (saveName || "").trim() || "SaveApp";
    const meta = await saveSnapshotToFile(nm, stateRef.current);
    if (meta) openToast("✅", "Kayıt dosyası hazır. Kaydet/Paylaş ekranından yer seç.");
    else openToast("⚠️", "Dosya yazılamadı. Backup Code kullan.");
  }

  async function handleLoadAppFile() {
    const Picker = getDocumentPicker();
    const FS = getFileSystem();
    if (!Picker?.getDocumentAsync || !FS?.readAsStringAsync) {
      return Alert.alert("LoadApp (File)", "Dosya seçici bulunamadı. (expo-document-picker gerekli)");
    }
    try {
      ...
      const raw = await FS.readAsStringAsync(uri);
      const obj = safeParseJson(raw);
      if (!obj) return Alert.alert("Load başarısız", "Dosya okunamadı / JSON geçersiz.");

      const snap = obj?.state ? obj : { name: asset?.name || "FileSave", state: obj };
      ...
      if (!Array.isArray(merged.notes)) merged.notes = [];
      if (!Array.isArray(merged.expensePlanner)) merged.expensePlanner = [];
      if (!Array.isArray(merged.stockSymbols)) merged.stockSymbols = DEFAULT_STATE.stockSymbols;
      if (!Array.isArray(merged.stockQuotes)) merged.stockQuotes = [];
      if (!merged.gameScores) merged.gameScores = DEFAULT_STATE.gameScores;

      setS(merged);
      openToast("✅", `Loaded: ${snap.name || "File"}`);
      setSaveLoadOpen(false);
    } catch (e) {
      Alert.alert("Load başarısız", "Dosya seçilemedi veya okunamadı.");
    }
  }
...
function ExpensePlannerPro({ theme, t, S, setPatch, onToast }) {
  ...
  const addItem = () => {
    ...
    setPatch({ expensePlanner: [{ id: uid(), title: String(title).trim(), category, phase, currency, amount: amt, dueDate: "", priority: "", note: String(note || "").trim(), paid: false, paidAt: "", at: nowISO() }, ...list] });
  ...
  const addStarterTemplate = () => {
    ...
    const additions = template.filter((r) => !existing.has(`${r[0]}|${r[2]}`)).map((r) => ({ id: uid(), title: r[0], category: r[1], phase: r[2], currency: r[3], amount: r[4], priority: "", dueDate: "", note: "", paid: false, paidAt: "", at: nowISO() }));
      ...
      {sortedRows.map((row) => {
        ...
        return (
          <GlassCard key={row.symbol} theme={theme} style={{ padding: 14, marginTop: 12 }}>
            <View style={{ flexDirection: "row", alignItems: "flex-start", gap: 10 }}>
              <View style={{ width: 34, height: 34, borderRadius: 12, backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: theme.stroke, alignItems: "center", justifyContent: "center" }}>
                <Text style={{ color: theme.gold, fontWeight: "900", fontSize: 13 }}>{row.rank}</Text>
              </View>
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: "row", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        { k: "games", label: t.games || "Oyunlar", icon: "🎮", sub: "Mini oyunlar • eğlence • skor" },
...
function GamesHub({ theme, t, S, setPatch }) {
  const [selected, setSelected] = useState(null);

  const scores = S.gameScores || {};
  const items = [
    { k: "tap", icon: "⚡", title: "Tap Rush", sub: "30 sn • ne kadar hızlı?", best: scores.tapRush ? `${scores.tapRush} tap` : "—" },
    { k: "react", icon: "🎯", title: "Refleks Testi", sub: "Işık yanınca dokun", best: scores.reactionBestMs ? `${scores.reactionBestMs} ms` : "—" },
    { k: "math", icon: "🧠", title: "Quick Math", sub: "10 soru • hız+doğruluk", best: scores.quickMath ? `${scores.quickMath} doğru` : "—" },
    { k: "rps", icon: "✊", title: "Taş • Kağıt • Makas", sub: "AI vs • seri yap", best: scores.rpsWins ? `${scores.rpsWins} win` : "—" },
    { k: "memory", icon: "🃏", title: "Memory Match", sub: "Eşleştir • en hızlı süre", best: scores.memoryBestSec ? `${scores.memoryBestSec}s` : "—" },
    { k: "spin", icon: "🎡", title: "Lucky Spin", sub: "Şans çarkı • mini ödül", best: scores.luckySpin ? `${scores.luckySpin} puan` : "—" },
  ];

  const Header = (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>🎮 Oyunlar</Text>
      <Text style={{ color: theme.sub, marginTop: 6 }}>
        Kısa mola: mini oyunlar, skorlar kaydolur (SaveApp ile birlikte).
      </Text>
    </GlassCard>
  );
  ...
  if (!selected) {
    return (
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
        {Header}
        {items.map((g) => (
          <TouchableOpacity key={g.k} onPress={() => setSelected(g.k)} style={{ marginTop: 12 }}>
            <GlassCard theme={theme} style={{ padding: 14 }}>
              <View style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                <View style={{ width: 44, height: 44, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, alignItems: "center", justifyContent: "center" }}>
                  <Text style={{ fontSize: 20 }}>{g.icon}</Text>
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>{g.title}</Text>
                  <Text style={{ color: theme.sub, marginTop: 4 }}>{g.sub}</Text>
                </View>
                <View style={{ alignItems: "flex-end" }}>
                  <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>Best</Text>
                  <Text style={{ color: theme.gold, fontWeight: "900" }}>{g.best}</Text>
                </View>
              </View>
            </GlassCard>
          </TouchableOpacity>
  ...
  const backBtn = (
    <TouchableOpacity onPress={() => setSelected(null)} style={{ marginBottom: 10 }}>
      <View style={{ paddingVertical: 10, paddingHorizontal: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, alignSelf: "flex-start" }}>
        <Text style={{ color: theme.text, fontWeight: "900" }}>← Liste</Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      {backBtn}
      {selected === "tap" && <GameTapRush theme={theme} S={S} setPatch={setPatch} />}
      {selected === "react" && <GameReaction theme={theme} S={S} setPatch={setPatch} />}
      {selected === "math" && <GameQuickMath theme={theme} S={S} setPatch={setPatch} />}
      {selected === "rps" && <GameRPS theme={theme} S={S} setPatch={setPatch} />}
      {selected === "memory" && <GameMemory theme={theme} S={S} setPatch={setPatch} />}
      {selected === "spin" && <GameLuckySpin theme={theme} S={S} setPatch={setPatch} />}
    </ScrollView>
  );
}

function GameTapRush({ theme, S, setPatch }) {
  const best = S.gameScores?.tapRush || 0;
  ...
  useEffect(() => {
    if (!running && tleft === 0) {
      if (score > best) setPatch({ gameScores: { ...(S.gameScores || {}), tapRush: score } });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [running, tleft]);

  const start = () => { setScore(0); setTleft(30); setRunning(true); };

  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>⚡ Tap Rush</Text>
      <Text style={{ color: theme.sub, marginTop: 6 }}>30 saniye boyunca basabildiğin kadar bas.</Text>

      <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
        <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>Süre</Text>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18, marginTop: 4 }}>{tleft}s</Text>
        </View>
        <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>Skor</Text>
          <Text style={{ color: theme.gold, fontWeight: "900", fontSize: 18, marginTop: 4 }}>{score}</Text>
        </View>
      </View>

      <TouchableOpacity
        onPress={() => { if (running) setScore((p) => p + 1); }}
        disabled={!running}
        style={{ marginTop: 14, opacity: running ? 1 : 0.6 }}
      >
        <View style={{ paddingVertical: 16, borderRadius: 18, borderWidth: 1, borderColor: theme.gold, backgroundColor: "rgba(255,209,112,0.18)", alignItems: "center" }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>{running ? "TAP!" : "Başlamak için Start"}</Text>
        </View>
      </TouchableOpacity>
      ...
      <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
        ...
        <View style={{ flex: 1, paddingVertical: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, alignItems: "center" }}>
          <Text style={{ color: theme.sub, fontWeight: "900" }}>Best: <Text style={{ color: theme.gold }}>{best}</Text></Text>
        </View>
      </View>
    </GlassCard>
  );
...
function GameReaction({ theme, S, setPatch }) {
  const best = S.gameScores?.reactionBestMs;
  const [stage, setStage] = useState("idle"); // idle, wait, go, done
  const [msg, setMsg] = useState("Start'e bas");
  ...
  const start = () => {
    ...
    setStage("wait");
    setMsg("Hazır… bekle…");
    const delay = 900 + Math.floor(Math.random() * 2400);
  ...
  const tap = () => {
    if (stage === "wait") {
      setStage("done");
      setMsg("Erken! 😅 Tekrar dene.");
      setMs(null);
      return;
    }
    if (stage === "go") {
      const v = Date.now() - goAtRef.current;
      setStage("done");
      setMs(v);
      setMsg(`${v} ms`);
      if (!best || v < best) setPatch({ gameScores: { ...(S.gameScores || {}), reactionBestMs: v } });
    }
  };

  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>🎯 Refleks Testi</Text>
      <Text style={{ color: theme.sub, marginTop: 6 }}>Işık “ŞİMDİ” dediğinde dokun. Erken dokunursan yanarsın.</Text>

      <TouchableOpacity onPress={tap} style={{ marginTop: 14 }}>
        <View style={{ paddingVertical: 22, borderRadius: 18, borderWidth: 1, borderColor: theme.stroke, backgroundColor: stage === "go" ? "rgba(123,255,178,0.16)" : theme.glass2, alignItems: "center" }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>{msg}</Text>
        </View>
      </TouchableOpacity>
      ...
      <Text style={{ color: theme.sub, marginTop: 12, fontWeight: "900" }}>Best: <Text style={{ color: theme.gold }}>{best ? `${best} ms` : "—"}</Text></Text>
    </GlassCard>
  );
}

function GameQuickMath({ theme, S, setPatch }) {
  const best = S.gameScores?.quickMath || 0;
  ...
  const submit = () => {
    if (!running || !q) return;
    const v = parseInt(String(ans).replace(/[^\d-]/g, ""), 10);
    if (Number.isFinite(v) && v === q.res) setScore((p) => p + 1);
    setLeft((p) => {
      const next = p - 1;
      if (next <= 0) {
        setRunning(false);
  ...
  useEffect(() => {
    if (!running && left === 0) {
      if (score > best) setPatch({ gameScores: { ...(S.gameScores || {}), quickMath: score } });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [running, left]);
  ...
  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>🧠 Quick Math</Text>
      <Text style={{ color: theme.sub, marginTop: 6 }}>10 soru. Hızlı cevapla.</Text>

      <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
        <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>Kalan</Text>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18, marginTop: 4 }}>{left}</Text>
        </View>
        <View style={{ flex: 1, padding: 10, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2 }}>
          <Text style={{ color: theme.sub, fontWeight: "800", fontSize: 12 }}>Doğru</Text>
          <Text style={{ color: theme.gold, fontWeight: "900", fontSize: 18, marginTop: 4 }}>{score}</Text>
        </View>
      </View>

      <View style={{ marginTop: 14, padding: 14, borderRadius: 18, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.06)", alignItems: "center" }}>
      ...
      <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
        <TextInput value={ans} onChangeText={setAns} editable={running} keyboardType="numeric" placeholder="Cevap" placeholderTextColor={theme.sub} style={{ flex: 1, padding: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, color: theme.text, backgroundColor: theme.glass2, fontWeight: "900" }} />
        <TouchableOpacity onPress={submit} disabled={!running} style={{ opacity: running ? 1 : 0.6 }}>
          <View style={{ paddingVertical: 12, paddingHorizontal: 14, borderRadius: 14, backgroundColor: "rgba(76,190,255,0.14)", borderWidth: 1, borderColor: theme.stroke, alignItems: "center" }}>
...
function GameRPS({ theme, S, setPatch }) {
  const best = S.gameScores?.rpsWins || 0;
  ...
  const pick = (p) => {
    ...
    let res = "Berabere";
    let win = false;
    if (p === ai) res = "Berabere";
    else if ((p === "Taş" && ai === "Makas") || (p === "Kağıt" && ai === "Taş") || (p === "Makas" && ai === "Kağıt")) { res = "Kazandın"; win = true; }
    else res = "Kaybettin";
  ...
  useEffect(() => {
    if (wins > best) setPatch({ gameScores: { ...(S.gameScores || {}), rpsWins: wins } });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [wins]);

  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>✊ Taş • Kağıt • Makas</Text>
      <Text style={{ color: theme.sub, marginTop: 6 }}>Seri yap (üst üste kazan).</Text>

      <View style={{ marginTop: 14, padding: 14, borderRadius: 18, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.06)" }}>
        <Text style={{ color: theme.text, fontWeight: "900" }}>{msg}</Text>
        <Text style={{ color: theme.sub, marginTop: 6, fontWeight: "900" }}>Seri: <Text style={{ color: theme.gold }}>{wins}</Text> • Best: <Text style={{ color: theme.gold }}>{best}</Text></Text>
      </View>

      <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
        {["Taş", "Kağıt", "Makas"].map((p) => (
          <TouchableOpacity key={p} onPress={() => pick(p)} style={{ flex: 1 }}>
            <View style={{ paddingVertical: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, alignItems: "center" }}>
              <Text style={{ color: theme.text, fontWeight: "900" }}>{p}</Text>
            </View>
          </TouchableOpacity>
        ))}
      </View>
...
function GameMemory({ theme, S, setPatch }) {
  const best = S.gameScores?.memoryBestSec;
  ...
  const newGame = () => {
    const cards = [...emojis, ...emojis].sort(() => Math.random() - 0.5).map((e, i) => ({ id: i, e, ok: false, open: false }));
    setDeck(cards);
    setOpenIdx(null);
    setLock(false);
    setDone(false);
  ...
  useEffect(() => {
    ...
    if (!best || sec < best) setPatch({ gameScores: { ...(S.gameScores || {}), memoryBestSec: sec } });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [done]);
  ...
  const tap = (idx) => {
    if (lock || done) return;
    const c = deck[idx];
    if (!c || c.ok || c.open) return;

    const next = deck.map((x, i) => i === idx ? { ...x, open: true } : x);
    setDeck(next);
    ...
    const b = next[idx];
    if (a.e === b.e) {
      const okd = next.map((x) => x.open ? { ...x, ok: true } : x);
      setDeck(okd);
      setOpenIdx(null);
      if (okd.every((x) => x.ok)) setDone(true);
    } else {
      setLock(true);
      setTimeout(() => {
        setDeck((cur) => cur.map((x, i) => (i === openIdx || i === idx) ? { ...x, open: false } : x));
        setOpenIdx(null);
        setLock(false);
      }, 700);
    }
  ...
  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>🃏 Memory Match</Text>
      <Text style={{ color: theme.sub, marginTop: 6 }}>Çiftleri bul. Ne kadar hızlı?</Text>

      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 12 }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Best: <Text style={{ color: theme.gold }}>{best ? `${best}s` : "—"}</Text></Text>
        <TouchableOpacity onPress={newGame}><Text style={{ color: theme.gold, fontWeight: "900" }}>Yeni Oyun</Text></TouchableOpacity>
      </View>

      <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10, marginTop: 12 }}>
        {deck.map((c, i) => (
          <TouchableOpacity key={c.id} onPress={() => tap(i)} style={{ width: "22%" }}>
            <View style={{ height: 56, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: c.open || c.ok ? "rgba(255,255,255,0.08)" : theme.glass2, alignItems: "center", justifyContent: "center" }}>
              <Text style={{ fontSize: 20 }}>{(c.open || c.ok) ? c.e : "•"}</Text>
            </View>
          </TouchableOpacity>
        ))}
      </View>
      ...
      {done && (
        <View style={{ marginTop: 14, padding: 12, borderRadius: 16, borderWidth: 1, borderColor: "rgba(123,255,178,0.35)", backgroundColor: "rgba(123,255,178,0.10)" }}>
          <Text style={{ color: theme.text, fontWeight: "900" }}>Tamamlandı! 🎉</Text>
        </View>
      )}
    </GlassCard>
  );
...
function GameLuckySpin({ theme, S, setPatch }) {
  const best = S.gameScores?.luckySpin || 0;
  const [score, setScore] = useState(0);
  const spin = useRef(new Animated.Value(0)).current;

  const doSpin = () => {
    spin.setValue(0);
    const reward = [10, 20, 40, 60, 80, 120][Math.floor(Math.random() * 6)];
    Animated.timing(spin, { toValue: 1, duration: 900, useNativeDriver: true }).start(() => {
      setScore((p) => {
        const next = p + reward;
        if (next > best) setPatch({ gameScores: { ...(S.gameScores || {}), luckySpin: next } });
        return next;
      });
    });
  };
  ...
  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>🎡 Lucky Spin</Text>
      <Text style={{ color: theme.sub, marginTop: 6 }}>Çarkı çevir, puan topla.</Text>

      <View style={{ alignItems: "center", marginTop: 16 }}>
        <Animated.View style={{ width: 120, height: 120, borderRadius: 60, borderWidth: 1, borderColor: theme.stroke, backgroundColor: "rgba(255,255,255,0.06)", alignItems: "center", justifyContent: "center", transform: [{ rotate: rot }] }}>
          <Text style={{ fontSize: 34 }}>🧿</Text>
        </Animated.View>
        <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "900" }}>Skor: <Text style={{ color: theme.gold }}>{score}</Text> • Best: <Text style={{ color: theme.gold }}>{best}</Text></Text>
      </View>

      <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
        <TouchableOpacity onPress={doSpin} style={{ flex: 1 }}>
          <View style={{ paddingVertical: 12, borderRadius: 14, backgroundColor: "rgba(255,209,112,0.18)", borderWidth: 1, borderColor: theme.gold, alignItems: "center" }}>
            <Text style={{ color: theme.text, fontWeight: "900" }}>SPIN</Text>
          </View>
        </TouchableOpacity>
        <TouchableOpacity onPress={() => setScore(0)} style={{ flex: 1 }}>
          <View style={{ paddingVertical: 12, borderRadius: 14, backgroundColor: theme.glass2, borderWidth: 1, borderColor: theme.stroke, alignItems: "center" }}>
            <Text style={{ color: theme.text, fontWeight: "900" }}>Reset</Text>
          </View>
        </TouchableOpacity>
      ...
      <Pressable onPress={onClose} style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.55)", padding: 16, justifyContent: "center" }}>
        <Pressable onPress={() => {}} style={{ maxHeight: "92%", backgroundColor: "rgba(10,16,24,0.94)", borderRadius: 18, padding: 16, borderWidth: 1, borderColor: theme.stroke }}>
          ...
          <Text style={{ color: theme.sub, marginTop: 8 }}>
            📁 SaveApp (File) ile dosya oluşturur, telefonun “Kaydet/Paylaş” ekranından istediğin yere kaydedersin. LoadApp (File) ile cihazdan dosya seçip geri yüklersin.
          </Text>
