...
function getFileSystem() {
  // Disabled: avoid bundling expo file system which can break GitHub APK builds if not installed.
  return null;
}
...
async function ensureKvDir() { return false; }
...
const INTRO_BG = require("./assets/intro_bg.jpg");
...
const [tab, setTab] = useState("home");
  ...
  const refreshSaveList = useCallback(async () => {
    ...
    const merged = snaps.map((x) => ({ id: x.id, name: x.name, at: x.at, version: x.version, source: "local" }));
    setSaveList(merged);
  }, []);
