...
function DubaiSkylineHero({ theme, versionInfo, lastNotification }) {
  ...
  const notifTimeText = lastNotification?.at ? new Date(lastNotification.at).toLocaleString() : "Bekleniyor";
  const titleText = lastNotification?.title || "Henüz bildirim yok";
  const msgText = lastNotification?.msg || "Sistem, haber ve başarı bildirimleri burada görünecek.";

  return (
    <GlassCard theme={theme} style={{ marginTop: 12, marginBottom: 12, padding: 0, overflow: "hidden" }}>
      <View style={{ minHeight: 164, padding: 14, paddingBottom: 56 }}>
        ...
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 16 }}>🌆 Dubai Skyline Theme • Sistem Bilgi Paneli</Text>
        <Text style={{ color: theme.sub, marginTop: 6, fontWeight: "800" }}>
          Versiyon: <Text style={{ color: theme.gold, fontWeight: "900" }}>{versionInfo?.versionName || "6.0.0"}</Text>
          {"  "}• Kod: <Text style={{ color: theme.gold, fontWeight: "900" }}>{String(versionInfo?.versionCode ?? 6)}</Text>
          {"  "}• {versionInfo?.label || "Final v6"}
        </Text>
        <Text style={{ color: theme.sub, marginTop: 2, fontWeight: "800", fontSize: 12 }}>
          Save şema: {versionInfo?.saveSchema || "v6"} • Uygulama: {versionInfo?.appName || "Dubai Journey PRO"}
        </Text>

        <View style={{ marginTop: 12, borderRadius: 14, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, padding: 10 }}>
          <Text style={{ color: theme.sub, fontSize: 11, fontWeight: "900", letterSpacing: 0.4 }}>SON BİLDİRİM</Text>
          <Text style={{ color: theme.text, fontWeight: "900", marginTop: 5 }} numberOfLines={1}>{titleText}</Text>
          <Text style={{ color: theme.sub, marginTop: 4, lineHeight: 17 }} numberOfLines={2}>{msgText}</Text>
          <Text style={{ color: theme.gold, marginTop: 6, fontWeight: "900", fontSize: 11 }}>Zaman: {notifTimeText}</Text>
        </View>
