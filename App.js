...
// ✅ Native file picker / sharing (optional deps)
...
async function handleSaveAppFile() {
  ...
  // Web: download
  if (IS_WEB) {
    const fn = `DubaiApp_${payload.name.replace(/\s+/g, "_")}_${payload.at.slice(0, 19).replace(/[:T]/g, "-")}.json`;
    downloadJsonFile(fn, payload);
    openToast("✅", "Dosya indirildi (.json)");
    return;
  }

  // Native: write to cache then open share sheet (user chooses where to save)
  const FS = getFileSystem();
  if (!FS?.cacheDirectory || !FS?.writeAsStringAsync) {
    Alert.alert("Uyarı", "FileSystem bulunamadı. Backup Code Export/Import kullan.");
    return;
  }

  const filename = buildSaveFileName(payload.name || "SaveApp", at);
  const fileUri = FS.cacheDirectory + filename;

  try {
    await FS.writeAsStringAsync(fileUri, JSON.stringify(payload), { encoding: FS.EncodingType?.UTF8 || undefined });
  } catch {
    Alert.alert("Hata", "Dosya yazılamadı. Backup Code Export/Import kullan.");
    return;
  }

  const Sharing = getSharing();
  try {
    const ok = await (Sharing?.isAvailableAsync ? Sharing.isAvailableAsync() : Promise.resolve(false));
    if (ok && Sharing?.shareAsync) {
      await Sharing.shareAsync(fileUri, {
        mimeType: "application/json",
        dialogTitle: "DubaiApp • SaveApp File",
        UTI: "public.json",
      });
      openToast("✅", "Paylaşım ekranı açıldı (kaydı seçtiğin yere koy)");
    } else {
      openToast("✅", "Kayıt oluşturuldu (cache). Backup Code da var.");
    }
  } catch {
    openToast("✅", "Kayıt oluşturuldu (cache). Backup Code da var.");
  }
...
function normalizeMergedStateFromSnapshotOrBackup(obj) {
  // Accept both:
  // 1) Snapshot file: { id,name,at,version,state:{...} }
  // 2) Backup Code: { ... } -> unpackBackupCode()
  let st = null;
  if (obj && typeof obj === "object" && obj.state && typeof obj.state === "object") {
    st = obj.state;
  } else {
    st = unpackBackupCode(obj);
  }
...
async function handleLoadAppFile() {
  // Web: file picker -> text
  if (IS_WEB) {
    pickJsonFileWeb(async (txt) => {
      const obj = safeParseJson(txt);
      if (!obj) return Alert.alert("Hatalı", "JSON okunamadı.");
      const merged = normalizeMergedStateFromSnapshotOrBackup(obj);
      if (!merged) return Alert.alert("Hatalı", "Dosya formatı tanınmadı.");
      setS(merged);
      openToast("✅", "Yüklendi: dosya");
  ...
  if (!DP?.getDocumentAsync) {
    Alert.alert("Eksik", "expo-document-picker gerekli. Termux'ta: npx expo install expo-document-picker");
    return;
  }
  ...
  try {
    ...
    const FS = getFileSystem();
    if (!FS?.readAsStringAsync) {
      Alert.alert("Eksik", "FileSystem bulunamadı.");
      return;
    }
    ...
    if (!merged) return Alert.alert("Hatalı", "Dosya formatı tanınmadı.");
    setS(merged);
    openToast("✅", `Yüklendi: ${asset?.name || "dosya"}`);
    setSaveLoadOpen(false);
  } catch {
    Alert.alert("Hata", "Dosya seçme/yükleme başarısız.");
  }
}
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      ...
      <GlassCard theme={theme} style={{ padding: 14, marginTop: 12 }}>
        ...
        <Text style={{ color: theme.sub, marginTop: 6 }}>
          SaveApp File / LoadApp File + Backup Code Export/Import.
        </Text>
...
========================= */
const GAME_BEST_TAP_KEY = STORAGE_KEY + "__game_best_tap";
const GAME_BEST_DICE_KEY = STORAGE_KEY + "__game_best_dice";

function GamesHubScreen({ theme, onToast }) {
  const [selected, setSelected] = useState(null);

  const games = [
    { id: "tap", icon: "⚡", title: "Neon Tap Rush", desc: "10 saniyede en çok tıkla" },
    { id: "memory", icon: "🧠", title: "Memory Flip", desc: "Eşleştir ve bitir" },
    { id: "dice", icon: "🎲", title: "Lucky Dice Duel", desc: "Zarı at • AI'ı yen" },
  ];
  ...
  return (
    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 18 }}>
      <GlassCard theme={theme} style={{ padding: 14 }}>
        ...
        <Text style={{ color: theme.sub, marginTop: 6 }}>Kısa molalar için mini oyunlar. Hepsi offline çalışır.</Text>
      </GlassCard>

      {!selected && (
        <>
          {games.map((g) => (
            <TouchableOpacity key={g.id} onPress={() => setSelected(g.id)} style={{ marginTop: 12 }}>
              <GlassCard theme={theme} style={{ padding: 14 }}>
                <View style={{ flexDirection: "row", alignItems: "center" }}>
                  ...
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    ...
                    <Text style={{ color: theme.sub, marginTop: 4, fontWeight: "800" }}>{g.desc}</Text>
                  </View>
      ...
      {selected === "tap" && (
        <GameShell theme={theme} title="Neon Tap Rush" onBack={() => setSelected(null)}>
          <TapRushGame theme={theme} onToast={onToast} />
        </GameShell>
      )}

      {selected === "memory" && (
        <GameShell theme={theme} title="Memory Flip" onBack={() => setSelected(null)}>
          <MemoryFlipGame theme={theme} onToast={onToast} />
        </GameShell>
      )}

      {selected === "dice" && (
        <GameShell theme={theme} title="Lucky Dice Duel" onBack={() => setSelected(null)}>
          <DiceDuelGame theme={theme} onToast={onToast} />
        </GameShell>
      )}
    </ScrollView>
  );
...
function GameShell({ theme, title, onBack, children }) {
  return (
    <View style={{ marginTop: 12 }}>
      <GlassCard theme={theme} style={{ padding: 14 }}>
        <TouchableOpacity onPress={onBack}>
          <Text style={{ color: theme.gold, fontWeight: "900" }}>← Oyunlar</Text>
        </TouchableOpacity>
        <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18, marginTop: 6 }}>{title}</Text>
      </GlassCard>
...
function TapRushGame({ theme, onToast }) {
  ...
  useEffect(() => {
    (async () => {
      const raw = await kvGet(GAME_BEST_TAP_KEY);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      if (!alive) return;
      const t = 10 - (Date.now() - started) / 1000;
      const tl = Math.max(0, Math.ceil(t));
      setTimeLeft(tl);
      if (t <= 0) {
        alive = false;
        clearInterval(timer);
        ...
        setTimeLeft(0);
        setBest((p) => {
          const nb = Math.max(p || 0, score);
          kvSet(GAME_BEST_TAP_KEY, String(nb)).catch(() => {});
          return nb;
        });
        onToast?.("🏁", `Bitti! Skor: ${score}`);
      }
    }, 120);
    return () => { alive = false; clearInterval(timer); };
  }, [running, score, onToast]);

  const start = () => {
    setScore(0);
    setTimeLeft(10);
    setRunning(true);
    onToast?.("⚡", "Başla! 10 saniye...");
  };

  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Süre</Text>
        <Text style={{ color: theme.text, fontWeight: "900" }}>{timeLeft}s</Text>
      </View>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 8 }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Skor</Text>
        <Text style={{ color: theme.text, fontWeight: "900" }}>{score}</Text>
      </View>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 8 }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Best</Text>
        <Text style={{ color: theme.gold, fontWeight: "900" }}>{best}</Text>
      </View>
      ...
      <TouchableOpacity
        ...
        style={{ marginTop: 14 }}
      >
        <View style={{ paddingVertical: 18, borderRadius: 18, backgroundColor: running ? "rgba(76,190,255,0.18)" : "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: theme.stroke, alignItems: "center" }}>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 18 }}>{running ? "TAP!" : "Başlamak için Start"}</Text>
        </View>
      </TouchableOpacity>
      ...
      <View style={{ flexDirection: "row", gap: 10, marginTop: 12 }}>
        ...
        <TouchableOpacity onPress={() => { setRunning(false); setTimeLeft(10); }} style={{ flex: 1 }}>
          <View style={{ paddingVertical: 12, borderRadius: 14, backgroundColor: theme.glass2, borderWidth: 1, borderColor: theme.stroke, alignItems: "center" }}>
      ...
      <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800", fontSize: 12 }}>
        Start’a bas, sonra ortadaki butona seri tıkla.
      </Text>
    </GlassCard>
  );
}

function MemoryFlipGame({ theme, onToast }) {
  const EMOJIS = ["🦅","🐉","💎","🏙️","⚡","🪙","🌙","🔥","🎯","🧠","🎲","🚀"];
  ...
  const newGame = () => {
    const base = EMOJIS.slice(0, 6);
    const cards = [...base, ...base].map((e, i) => ({ id: String(i) + "_" + e, e }));
  ...
  const tap = (idx) => {
    ...
    if (next.length === 2) {
      setMoves((p) => p + 1);
      const [a, b] = next;
      const same = deck[a]?.e && deck[a]?.e === deck[b]?.e;
      if (same) {
        setMatched((p) => ({ ...p, [a]: true, [b]: true }));
        setOpen([]);
        const doneCount = Object.keys({ ...matched, [a]: true, [b]: true }).length;
        if (doneCount + 0 >= deck.length) {
          setTimeout(() => onToast?.("🏁", `Bitti! Hamle: ${moves + 1}`), 60);
        }
      } else {
  ...
  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Hamle</Text>
        <Text style={{ color: theme.text, fontWeight: "900" }}>{moves}</Text>
      </View>
      <View style={{ flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", marginTop: 12 }}>
        {deck.map((c, idx) => {
          const show = open.includes(idx) || matched[idx];
          return (
            <TouchableOpacity key={c.id} onPress={() => tap(idx)} style={{ width: "31%", marginBottom: 10 }} activeOpacity={0.85}>
              <View style={{ height: 72, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: show ? "rgba(255,209,112,0.14)" : theme.glass2, alignItems: "center", justifyContent: "center" }}>
                <Text style={{ fontSize: 28 }}>{show ? c.e : "❓"}</Text>
              </View>
            </TouchableOpacity>
          );
        })}
      ...
      <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800", fontSize: 12 }}>
        6 çift kart var. İki kart seç, eşleşirse açık kalır.
      </Text>
    </GlassCard>
  );
}

function DiceDuelGame({ theme, onToast }) {
  ...
  useEffect(() => {
    (async () => {
      const raw = await kvGet(GAME_BEST_DICE_KEY);
      const n = raw ? Number(raw) : 0;
      setBest(Number.isFinite(n) ? n : 0);
    })();
  }, []);
  ...
  const roll = () => {
    ...
    if (y > a) {
      setStreak((p) => {
        const ns = p + 1;
        setBest((b) => {
          const nb = Math.max(b || 0, ns);
          kvSet(GAME_BEST_DICE_KEY, String(nb)).catch(() => {});
          return nb;
        });
        onToast?.("🎉", `Kazandın! Seri: ${ns}`);
        return ns;
      });
    } else if (y < a) {
      setStreak(0);
      onToast?.("😅", "Kaybettin. Seri sıfırlandı");
    } else {
      onToast?.("🤝", "Berabere!");
    }
  };
  ...
  return (
    <GlassCard theme={theme} style={{ padding: 14 }}>
      <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Seri</Text>
        <Text style={{ color: theme.text, fontWeight: "900" }}>{streak}</Text>
      </View>
      <View style={{ flexDirection: "row", justifyContent: "space-between", marginTop: 8 }}>
        <Text style={{ color: theme.sub, fontWeight: "900" }}>Best</Text>
        <Text style={{ color: theme.gold, fontWeight: "900" }}>{best}</Text>
      ...
      <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
        <View style={{ flex: 1, padding: 12, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, alignItems: "center" }}>
          <Text style={{ color: theme.sub, fontWeight: "900" }}>Sen</Text>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 34, marginTop: 6 }}>{you || "—"}</Text>
        </View>
        <View style={{ flex: 1, padding: 12, borderRadius: 16, borderWidth: 1, borderColor: theme.stroke, backgroundColor: theme.glass2, alignItems: "center" }}>
          <Text style={{ color: theme.sub, fontWeight: "900" }}>AI</Text>
          <Text style={{ color: theme.text, fontWeight: "900", fontSize: 34, marginTop: 6 }}>{ai || "—"}</Text>
        </View>
      </View>

      <TouchableOpacity onPress={roll} style={{ marginTop: 12 }}>
        <View style={{ paddingVertical: 12, borderRadius: 14, backgroundColor: "rgba(255,209,112,0.18)", borderWidth: 1, borderColor: theme.gold, alignItems: "center" }}>
          <Text style={{ color: theme.text, fontWeight: "900" }}>Zar At</Text>
        </View>
      </TouchableOpacity>

      <Text style={{ color: theme.sub, marginTop: 10, fontWeight: "800", fontSize: 12 }}>
        Amaç: AI’dan büyük atıp seri yap.
      </Text>
    </GlassCard>
  );
}
  ...
  const sections = [
    ...
    },
    {
      ...
      items: [
        ...
        { k: "games", label: "Oyunlar", icon: "🎮", sub: "Mini oyunlar • eğlence • kısa mola" },
      ...
      <Pressable onPress={onClose} style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.55)", padding: 16, justifyContent: "center" }}>
        <Pressable onPress={() => {}} style={{ maxHeight: "92%", backgroundColor: "rgba(10,16,24,0.94)", borderRadius: 18, padding: 16, borderWidth: 1, borderColor: theme.stroke }}>
          ...
          <Text style={{ color: theme.sub, marginTop: 8, lineHeight: 18 }}>
            ✅ <Text style={{ fontWeight: "900", color: theme.text }}>SaveApp File</Text> ile kayıt dosyasını dışarıya çıkarırsın (paylaşım ekranı açılır).
            {"
"}✅ <Text style={{ fontWeight: "900", color: theme.text }}>LoadApp File</Text> ile telefondan .json seçip geri yüklersin.
            {"
"}⚡ Alternatif: Backup Code Export/Import (kopyala-yapıştır, her yerde çalışır).
          </Text>
          ...
          <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
            <TouchableOpacity onPress={onSaveFile} style={{ flex: 1 }}>
              <View style={{ paddingVertical: 12, borderRadius: 14, backgroundColor: "rgba(255,209,112,0.16)", borderWidth: 1, borderColor: theme.gold, alignItems: "center" }}>
                <Text style={{ color: theme.text, fontWeight: "900" }}>SaveApp File</Text>
              </View>
            </TouchableOpacity>
            <TouchableOpacity onPress={onLoadFile} style={{ flex: 1 }}>
              <View style={{ paddingVertical: 12, borderRadius: 14, backgroundColor: "rgba(255,255,255,0.06)", borderWidth: 1, borderColor: theme.stroke, alignItems: "center" }}>
                <Text style={{ color: theme.text, fontWeight: "900" }}>LoadApp File</Text>
              </View>
            </TouchableOpacity>
          </View>
