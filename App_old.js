// App.js — Dubai DrabornEagle • Dubai Journey PRO v5.3 (Single-file • No external dependencies)
// ✅ NEW: Modern Dubai intro / professional loading + brand logo hook (replace assets/icon.png)
...
// =========================
// Brand (Logo + App Display Name)
// =========================
// ✅ Ekteki görseli kullanmak için: proje içinde ./assets/icon.png dosyasını bu görselle değiştir.
// Böylece hem intro’da logo görünür, hem de Expo/EAS ikon ayarın icon.png üzerinden kolay olur.
const APP_DISPLAY_NAME = "Dubai DrabornEagle";
const BRAND_LOGO = (() => {
  try {
    // Expo template’lerinde bu dosya zaten vardır (assets/icon.png)
...
// ✅ AsyncStorage (installed dependency)
...
/* =========================
   Intro Splash — Dubai Futuristic Loading (No deps)
   - Premium, modern Dubai vibe: neon halo, skyline silhouette, particles, scanline, typewriter
   - Plays at least MIN_MS, then fades out when `ready`
   - Logo uses BRAND_LOGO (replace ./assets/icon.png)
========================= */
function IntroSplash({ theme, logo, ready, onDone }) {
  const MIN_MS = 5000; // auto-advance after 5s // premium intro minimum duration (ms)
  const startedAt = useRef(Date.now());

  const opacity = useRef(new Animated.Value(1)).current;
  const scale = useRef(new Animated.Value(0.92)).current;

  const halo = useRef(new Animated.Value(0)).current;      // rotate
  const pulse = useRef(new Animated.Value(0)).current;     // logo pulse
  const scan = useRef(new Animated.Value(0)).current;      // scanline
  const prog = useRef(new Animated.Value(0)).current;      // progress bar
  const shimmer = useRef(new Animated.Value(0)).current;   // subtle shimmer

  const [typedTop, setTypedTop] = useState("");
  const [typedBottom, setTypedBottom] = useState("");
  const [status, setStatus] = useState("Booting modules…");
  const minDone = useRef(false);
  const finished = useRef(false);

  const TOP = "DRABORNEAGLE";
  const BOTTOM = "DUBAI SYSTEM ONLINE";

  const tryFinish = useCallback(() => {
    if (finished.current) return;
    if (!minDone.current) return;
    finished.current = true;

    Animated.parallel([
      Animated.timing(opacity, { toValue: 0, duration: 650, easing: Easing.inOut(Easing.cubic), useNativeDriver: true }),
    ...
    const t0 = setInterval(() => {
      ...
      if (i >= TOP.length) {
        ...
        const t1 = setInterval(() => {
          ...
          if (j >= BOTTOM.length) clearInterval(t1);
        }, 35);
      }
    }, 55);
  ...
  }, [MIN_MS, halo, pulse, scan, prog, shimmer, particleAnim, particles, tryFinish]);

  useEffect(() => {
    tryFinish();
  }, [ready, tryFinish]);

  const haloRot = halo.interpolate({ inputRange: [0, 1], outputRange: ["0deg", "360deg"] });
  const logoScale = pulse.interpolate({ inputRange: [0, 1], outputRange: [1, 1.03] });
  const scanY = scan.interpolate({ inputRange: [0, 1], outputRange: [-18, 210] });
  const shimmerX = shimmer.interpolate({ inputRange: [0, 1], outputRange: [-220, 220] });

  const pFill = prog.interpolate({ inputRange: [0, 1], outputRange: ["0%", "100%"] });

  const bg = theme?.bg || "#070A12";
  const fg = theme?.text || "#EAF2FF";
  const sub = theme?.sub || "rgba(234,242,255,0.72)";

  return (
    <View style={{ position: "absolute", left: 0, right: 0, top: 0, bottom: 0, backgroundColor: bg, zIndex: 9999 }}>
      <StatusBar barStyle="light-content" />

      {/* Background: gradient-ish layers + vignette */}
      <View style={{ position: "absolute", left: 0, right: 0, top: 0, bottom: 0 }}>
        <View style={{ position: "absolute", left: 0, right: 0, top: 0, height: "62%", backgroundColor: "#0A1022", opacity: 0.9 }} />
        <View style={{ position: "absolute", left: 0, right: 0, top: "38%", bottom: 0, backgroundColor: "#05070E" }} />
        <View style={{ position: "absolute", left: -140, top: -160, width: 360, height: 360, borderRadius: 360, backgroundColor: "#2A7BFF", opacity: 0.12 }} />
        <View style={{ position: "absolute", right: -160, top: 40, width: 420, height: 420, borderRadius: 420, backgroundColor: "#00E7FF", opacity: 0.08 }} />
        <View style={{ position: "absolute", left: -200, bottom: -220, width: 520, height: 520, borderRadius: 520, backgroundColor: "#FFB000", opacity: 0.06 }} />
        <View style={{ position: "absolute", left: 0, right: 0, top: 0, bottom: 0, backgroundColor: "#000", opacity: 0.18 }} />
      </View>

      {/* Particles */}
      <View style={{ position: "absolute", left: 0, right: 0, top: 0, bottom: 0 }}>
        {particles.map((p, idx) => {
          const t = particleAnim[idx].interpolate({ inputRange: [0, 1], outputRange: [0, 1] });
          const tx = t.interpolate({ inputRange: [0, 1], outputRange: [0, p.dx] });
          const ty = t.interpolate({ inputRange: [0, 1], outputRange: [0, p.dy] });
          return (
            <Animated.View
              key={p.id}
              style={{
      ...
      {/* Content */}
      <Animated.View style={{ flex: 1, alignItems: "center", justifyContent: "center", opacity, transform: [{ scale }, { translateY: -10 }] }}>
        {/* Halo + logo */}
        <View style={{ width: 170, height: 170, alignItems: "center", justifyContent: "center" }}>
          ...
          <Animated.View
            style={{
              ...
              borderRadius: 142,
              borderWidth: 2,
              borderColor: "rgba(42,123,255,0.20)",
              transform: [{ rotate: haloRot }],
            }}
          />
          <Animated.View style={{ transform: [{ scale: logoScale }] }}>
            ...
            >
              <Image source={logo} resizeMode="cover" style={{ width: 112, height: 112 }} />
            </View>
          </Animated.View>
          ...
          <Animated.View
            ...
            style={{
              ...
              height: 10,
              borderRadius: 12,
              backgroundColor: "rgba(0,231,255,0.20)",
        ...
        <View style={{ marginTop: 18, alignItems: "center" }}>
          <Text style={{ color: fg, fontWeight: "900", letterSpacing: 3, fontSize: 22 }}>
            {typedTop || " "}
          </Text>
          <Text style={{ color: sub, marginTop: 6, fontWeight: "700", letterSpacing: 2, fontSize: 12 }}>
            {typedBottom || " "}
          </Text>
        </View>

        {/* Dubai skyline silhouette */}
        <View style={{ marginTop: 22, width: 260, height: 64, overflow: "hidden" }}>
          <View style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: 64, backgroundColor: "rgba(255,255,255,0.03)" }} />
          <View style={{ position: "absolute", left: 8, bottom: 0, width: 18, height: 22, backgroundColor: "rgba(191,231,255,0.14)" }} />
          <View style={{ position: "absolute", left: 30, bottom: 0, width: 16, height: 34, backgroundColor: "rgba(191,231,255,0.18)" }} />
          <View style={{ position: "absolute", left: 52, bottom: 0, width: 22, height: 28, backgroundColor: "rgba(191,231,255,0.12)" }} />
          <View style={{ position: "absolute", left: 80, bottom: 0, width: 18, height: 44, backgroundColor: "rgba(191,231,255,0.22)" }} />
          {/* Burj-ish spike */}
          <View style={{ position: "absolute", left: 112, bottom: 0, width: 10, height: 58, backgroundColor: "rgba(0,231,255,0.18)" }} />
          <View style={{ position: "absolute", left: 126, bottom: 0, width: 20, height: 36, backgroundColor: "rgba(191,231,255,0.14)" }} />
          <View style={{ position: "absolute", left: 152, bottom: 0, width: 16, height: 26, backgroundColor: "rgba(191,231,255,0.10)" }} />
          <View style={{ position: "absolute", left: 174, bottom: 0, width: 20, height: 40, backgroundColor: "rgba(191,231,255,0.16)" }} />
          <View style={{ position: "absolute", left: 200, bottom: 0, width: 18, height: 30, backgroundColor: "rgba(191,231,255,0.12)" }} />
          <View style={{ position: "absolute", left: 222, bottom: 0, width: 22, height: 24, backgroundColor: "rgba(191,231,255,0.10)" }} />
          {/* glow base */}
          <View style={{ position: "absolute", left: 0, right: 0, bottom: -18, height: 28, backgroundColor: "#00E7FF", opacity: 0.08 }} />
        </View>

        {/* Status + progress */}
        <View style={{ marginTop: 18, width: "78%" }}>
          <Text style={{ color: "rgba(234,242,255,0.65)", fontSize: 12, fontWeight: "700", textAlign: "center" }}>
            {status}
          </Text>
          ...
          >
            <View style={{ height: "100%", width: pFill, backgroundColor: "rgba(0,231,255,0.55)" }} />
            <Animated.View
              pointerEvents="none"
              style={{
                position: "absolute",
          ...
          <Text style={{ marginTop: 10, color: "rgba(234,242,255,0.45)", fontSize: 11, textAlign: "center" }}>
            Tap to skip
          </Text>
        </View>
      </Animated.View>
...
export default function App() {
  ...
  const refreshSaveList = useCallback(async () => {
    ...
    setSaveList(snaps.map((x) => ({ id: x.id, name: x.name, at: x.at, version: x.version })));
  }, []);
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#000" }}>
      ...
      <ImageBackground source={{ uri: theme.skyline }} style={{ flex: 1 }} resizeMode="cover">
        <View style={{ flex: 1, backgroundColor: theme.overlay }}>
          ...
          <View style={{ flex: 1, paddingHorizontal: 14, paddingTop: 10, paddingBottom: 92, opacity: introOpen ? 0.35 : (boot ? 0.7 : 1) }}>
            {tab === "home" && (
      ...
      <View style={{ marginBottom: 10 }}>
        <View style={{ borderRadius: 18, borderWidth: 1, borderColor: theme.stroke, overflow: "hidden" }}>
          <View style={{ padding: 14, backgroundColor: "rgba(255,255,255,0.10)" }}>
            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
              <View>
                <Text style={{ color: theme.text, fontWeight: "900", fontSize: 17 }}>DrabornEagle</Text>
...
function TxnModal({ open, theme, t, type, categories, amount, setAmount, currency, setCurrency, category, setCategory, note, setNote, onClose, onSave }) {
...
/* Gold gram modal */
function GoldGramModal({ open, theme, t, grams, setGrams, goldGramTl, usdToTl, onClose, onSave }) {
  const g = toInt(grams);
      ...
      <Pressable onPress={onClose} style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.55)", padding: 16, justifyContent: "center" }}>
        <Pressable onPress={() => {}} style={{ maxHeight: "92%", backgroundColor: "rgba(10,16,24,0.94)", borderRadius: 18, padding: 16, borderWidth: 1, borderColor: theme.stroke }}>
          ...
          <View style={{ flexDirection: "row", gap: 10, marginTop: 10 }}>
            ...
            <TouchableOpacity onPress={onSaveFile} style={{ flex: 1, opacity: isWeb ? 1 : 0.45 }}>
